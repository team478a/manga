...
- MANGAIで全ページ確定・PDF保存後、販売用一式を作り、KDPまたはBOOTHへ本人が手動登録、プレビュー、公開、販売記録を行う流れを追加した。
...
- `test`アカウントでは管理画面が権限拒否されることを確認した。管理者sessionのread-only一覧確認と判定保存canaryは未実施。Provider、生成、Storage、credit、利用者コンテンツ変更なし。
...
- Implementation commit `a7e723d`を含むPR #511はmerge commit `4d1b0fa`でマージ済み。全CI／Vercel Preview成功済み。
...
- migrationはProductionへ適用済み。Provider、生成、credit、利用者Project／画像は変更していない。集中4/4、Hub 1013/1013、Desktop 407/407、Canvas 26/26、AI 50/50、a11y 29画面blocking violation 0、deps error 0（既知warning 2件）、lint、typecheck、Hub／Desktop build、migration 84/84、RC構造、diff check成功。
...
- Implementation commit `3ee155d`をpushし、Draft PR #508を作成した。全CI／Vercel Previewを確認中。
- 公式固定URLからComfyUI v0.34.0、SDXL base、SDXL VAE、Canny ControlNetをGit管理外へ取得し、正本の容量・SHA-256と全件一致を確認した。
- 内容非保持Bundle証跡から4artifactとrepository内4workflow／mapping digestをmanifestへ固定し、bundle preflightを`fixed=8 / pending=0`、release readinessを6/9 READYへ進めた。
- 実path、作品内容、Prompt、端末識別情報、秘密値はGitへ保存せず、local rootと証跡はcommitしない。未追跡`apps/desktop/artifacts/`も変更・commit対象外である。
- 現端末はコード署名秘密鍵とNVIDIA Runtimeがないため、署名、Runtime起動、生成、12GB実機証跡、Stage 0完了連結は行っていない。Production、Cloud、Provider、credit、配布、実候補者操作なし。
...
- 次はDraft PR #508の全CI／Vercel Preview成功確認である。その後は署名済み受入れ専用artifactと12GB技術モニター実機受入れを外部前提として進める。
...
- Implementation commit `4c7af68`を含むPR #507はmerge commit `746c399`でマージ済み。マージ後Required Quality run `35181913762`とDesktop Windows run `35181913763`も成功した。
...
- Implementation commit `0593e60`を含むPR #505はmerge commit `e18c99c`でマージ済み。マージ後Required Quality run `35176879178`とDesktop Windows run `35176879177`も成功した。
...
## 2026-09-17 Codex: Desktop Adult Stage 1運用ライフサイクル統合read-only監査

- PR #502 merge commit `cda5029`から`codex/adult-stage1-lifecycle-audit-20260917`を作成した。マージ後Required Quality run `35167341045`とDesktop Windows run `35167340992`は成功済み。
- Implementation commit `5cfcdbe`をpushし、Draft PR #503を作成した。全CI／Vercel Previewを確認中。
- 初回Stage 1招待証跡と、古い順に指定した0件以上の状態遷移proposalを連結して現在状態までread-only監査するCLIを追加した。
- 初回招待とのmonitor一致、各source／target台帳のbyte連続性、途中遷移の適用完了、最後の4適用状態、重複、欠落、別monitor混入、改変、監査中変更を検査する。既存個別監査は通常契約を維持し、receipt完成済みの過去工程だけ履歴検査できるよう内部拡張した。
- 招待台帳文書とStage 1 runbookへ統合監査コマンド、指定順、状態の意味、fail-closed条件、承認非代替境界を追記した。
...
- Production、Cloud、Provider、Runtime／model、生成、招待、配布、credit、実monitor・実承認・実proposal・実台帳操作なし。未追跡`apps/desktop/artifacts/`はcommit対象外。次はPR #503の全CI／Vercel Preview成功まで確認する。
...
- Implementation commit `8e3a492`を含むPR #501はmerge commit `e53e251`でマージ済み。マージ後Required Quality run `35165212620`とDesktop Windows run `35165212632`も成功した。
...
- Implementation commit `d5c0ae8`を含むPR #499はmerge commit `7a91a70`でマージ済み。マージ後Required Quality run `35122591058`とDesktop Windows run `35122591027`も成功した。
...
- Commit `8e80d10`をpushしてDraft PR #490を作成した。初回HEADでCore quality run `35070662348`（2分9秒）、Migration roundtrip（52秒）、Desktop Windows run `35070662457`（4分33秒）、Vercel Preview／Preview Commentsはすべて成功した。Production、Cloud、Provider、Runtime／model、生成、招待、配布、credit、証明書購入、実署名、Secret、Release、実候補者情報、実計画変更なし。未追跡`apps/desktop/artifacts/`は未commit。
- 次: 正本同期commitの最終CI／Vercel Preview成功まで確認して停止する。実計画とStage 0外部操作は別承認事項である。
...
- Production、Cloud、Provider、Runtime／model、生成、招待、配布、credit、証明書購入、実署名、Secret、Release操作なし。未追跡`apps/desktop/artifacts/`は未commit。次は正本同期commitの最終CI／Vercel Preview成功で停止する。実BundleとStage 0外部操作は別承認事項である。
...
- Production `test`の23ページをread-only再確認したところ、失敗履歴は「生成失敗・再実行可能」「このコマだけ再実行」のままだった。Production DBをManagement API経由の`SELECT`だけで照合し、23ページ最新失敗Job `6ed98da4-b43f-437f-b715-f45bfe347fb4`と24ページ最新失敗Job `5948a685-d41b-4b83-940c-bc9df828835c`はいずれもProvider Job IDあり、第1段階の一般向け再構成済み、第2段階の保守的再構成は未実行と確認した。
...
- 終端moderation失敗の見直し後も、従来は最小2案をJob作成前の費用確認なしで開始するため、低コストで慎重に再開する余地がなかった。
- 回復対象だけ1案を初期値にし、通常生成の候補数状態とは分離した。通常生成の2〜4案・初期3案は変えていない。
...
- 一括生成側で解消した終端moderationの行き止まりが、原稿編集画面の生成履歴には残っていた。第2段階の一般向け安全再構成まで拒否されたJobにも「このコマだけ再実行」が表示され、再実行して初めて拒否される状態だった。
...
- 集中16/16、Hub 645/645、Canvas 26/26、AI 50/50、Desktop 230/230、a11y 29画面0 violation、migration 82/82、deps error 0（既存warning 2件）、lint、typecheck、Hub／Desktop build、RC structure、diff check成功。初回CIが検出した既存Batch案内文の互換性違反は、新案内との併記で修正した。
- Production、報告状態、通知、作品、画像、Provider、Job、Asset、credit、生成なし。複数画像添付は別タスク。
...
- Base: `c81037c`（PR #459 merge commit）
...
## 2026-09-14 Codex: 品質確認Pilot 第三者裁定DB基盤

- Branch: `codex/feat-quality-review-adjudication-db-20260914`
- Base: `a9b803b`（PR #458 merge commit）
- 既存回答を保持したまま第三者裁定を追加保存するadditive migration、裁定専用table、payload非保持のappend-only event、RLS、管理者／本人別Security Definer RPCを実装した。
...
- Base: `c12bd8c`（PR #450 merge commit）。本線Required Quality run `34808654501`、Desktop Windows run `34808654502`は成功。
- Productionをread-only監査し、ネーム保存報告タスク`79a6b398-a344-4a70-add0-aa626e127daf`が`detected`、元報告`65f4c2a5-3852-4434-8599-a0f9f222dcd3`が`new`／`submitted`のまま残っていることを確認した。workflowは`storyboard`、priorityは`critical`、発生1件。
- 利用者通知は受付通知1件だけで、対応完了通知は0件。今回のネーム保存修正に該当する更新情報も未掲載だった。
- 修正migrationと`test`での4ページ保存受入れは完了済み。今回の監査では報告状態、通知、更新情報、作品、Provider、Job、Asset、credit、生成処理を変更していない。
- docs-only差分のRC構造検査とdiff check成功。外部環境preflightの既存PENDING項目は不変。
- 次はdocs-only Draft PRと全CI／Vercel Preview成功で停止する。報告の`resolved`更新、対応完了通知、更新情報掲載は責任者の実行時承認後に別工程で行う。
...
- Base: `1bfc720`（PR #442 merge commit）。本線Required Quality、Migration roundtrip、Desktop Windows build成功を確認した。
- 責任者の実行時承認後、Productionのシナリオ保存報告タスク`2954455c-42cd-4ad3-b830-335c3af3103e`を条件付きで`resolved`へ1件更新した。
- 元報告`426263cd-2325-442e-88d4-c442cdf3fcfd`も公開状態`resolved`となり、修正内容、Production受入結果、先行販売購入者5名、運営側検証の並行実施を説明する利用者向け`monitor_report_status`通知1件を確認した。
- 他3件の報告、作品、Provider、Job、Asset、credit、生成操作なし。次はdocs-only Draft PRと全CI／Vercel Preview成功で停止する。
...
- 二重明示flag、未記録manifest、atomic writeを必須とする承認記録commandを追加した。
- 集中2/2、readiness 4 READY／4 BLOCKED、Desktop 224/224、lint、typecheck、deps、Desktop build、a11y 29画面blocking violation 0、diff check成功。承認状態、Production、Cloud、Provider、Job、credit、生成、Release操作0件。
- 次: commit、push、Draft PR、全CI／Vercel Preview成功で停止する。
...
- エディター上部に残った日本語のコマンドパレットaria-label／titleを既存翻訳キーへ統一した。利用者がEnglish書き出しdialogの実音声を確認した。
...
- English生成ジョブ／書き出しはUI Automationで確認し、English書き出しdialogは利用者が実音声を確認した。
...
- Base: `origin/feature/manga-canvas-mvp`@`9297281`（PR #372 merge commit）
- Production project `vmdsyxykcrgxcdbrwlkv`へ未適用13 migration（`202608240001`〜`202608260002`）だけを順序通り適用し、主要schema／constraint、価格guard、Creator復旧を確認した。
- 固定一般向け作品のstyle参照を含むBFL `flux-2-pro` 1 Jobだけを実行。予約2 credits／`$0.180`、実額`$0.045`、差額`$0.135`解放、Asset 1件、attempt 1、同一idempotency 1件、queue 0で完了した。
- 既存Production Worker workflow run `33036852036`を1回だけ起動。追加Job、自動retry、失敗Job再実行、reference binding手動変更、秘密値変更、staging操作0件。
- 本ブランチはdocs-only。残る外部gateはstaging durable export受入れ1件。詳細は`docs/RELEASE_CANDIDATE_PRODUCTION_BFL_ACCEPTANCE_CLOSEOUT_20260827.md`。
- Hub 916/916、Canvas 26/26、AI 48/48、Desktop 182/182、a11y violation 0、migration 74件、deps、lint、全型検査、Hub／Desktop build、RC structure、diff check成功。`rc:preflight`の外部設定・手動E2E pendingは残るstaging gateと一致する。
