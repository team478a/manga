...
Stage 1の最初の1名は、`DESKTOP_ADULT_TECHNICAL_MONITOR_STAGE1_RUNBOOK_20260916.md`の一回限定招待承認を配布直前に消費してから手動配布する。消費receiptは配布を自動実行せず、台帳も自動更新しない。配布後にreceiptのrandom monitor ID、署名済みPilot version、実際の配布日時、確認済み環境帯を使って`INVITED` entryを追加し、本検査を実行する。candidate ID、承認fileのpath、氏名、メールは台帳へ転記しない。
