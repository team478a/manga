...
- プロパティ / レイヤー / AIタブ
- 最後に開いたタブの端末保存
- React PortalによるCanvas選択状態と既存操作ロジックの共有
- 選択オブジェクト設定とProject / Page / Asset情報のプロパティ統合
- レイヤー選択、表示、ロック、前面 / 背面、D&D並び替えの移設
- Canvasの表示メニューから右レイヤータブを開く導線
- Creator Chatのパネル化とCanvasを見ながらの送信、停止、再生成、コピー、Pageメモ保存
- チャット履歴、テンプレート、Project / Pageコンテキスト、AI設定導線の維持

### UI-5 状態・生成・書き出し（完了）

- 保存中、保存済み、書き出し中、キャンセル、失敗をヘッダーの共通状態表示へ統合
- Ollama / ComfyUIの有効・確認中・接続・失敗状態を下部ステータスへ表示
- AI接続状態の60秒更新、手動更新、設定画面への導線
- Project単位の生成ジョブを2秒間隔で更新し、実行中件数を表示
- 制作画面を離れず直近8件、進捗、失敗内容、キャンセルを確認できるDrawer
- 既存のAI生成画面へ移動する導線
