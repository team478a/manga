...
次は`UI-5 状態・生成・書き出し`へ進む。

App Shellの保存状態を基準に、Ollama / ComfyUI接続要約、生成ジョブ件数とDrawer、書き出しダイアログ・進捗・完了状態を制作画面へ統合する。既存のGeneration Jobs、AI設定、書き出しIPCを再利用し、常時表示する情報は最小限にする。
