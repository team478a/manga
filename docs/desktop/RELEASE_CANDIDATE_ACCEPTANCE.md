...
現行コードからWindows x64製品版を再作成し、受入れ用に複製したProjectへ短編8Page構成を追加して合計9Pageを書き出しました。確認ダイアログ、Page進捗、パッケージング、完了画面、表紙未設定警告は正常に表示されました。

生成PDFは9Page・384×576ptで、先頭・中間・最終Pageの白背景とコマ枠を画像化して確認しました。画像ZIPは`001.png`〜`009.png`の9件、販売パッケージは`mangai.sales-package` version 1、Page数9、sample 3件、表紙0件です。manifestに記録された8ファイルのbyte数とSHA-256はすべて実ファイルと一致しました。これにより、外部サービスを必要としないDesktopローカル受入れを完了とします。
