...
- バックアップ破損、DB復旧、詳細なworkflow検証など低頻度のmain processメッセージ

Hub URL・認証、AI Provider未設定、ComfyUI無効・timeout、低VRAM排他、診断送信、書き出し、OS資格情報暗号化、更新エラーなど、主要操作で表示されるmain process・外部サービスメッセージはrendererの選択localeへ変換します。IPCが付加する内部的な呼び出しprefixは画面表示前に除外します。
...
2026-07-16時点では日本語・英語の18画面・状態で合計392項目が合格し、自動判定の違反は0件です。書き出しダイアログ表示中は背景要素へ`inert`と`aria-hidden`を設定し、読み上げとkeyboard移動をダイアログ内へ限定します。
...
完全な英語版と全状態のWCAG評価は未完了です。残る低頻度main process messageを同じ変換層へ移行し、axe対象を画像生成の実行中・完了・失敗と各確認ダイアログへ広げ、Narratorによる手動確認を追加する必要があります。
