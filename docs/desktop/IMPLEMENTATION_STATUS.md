...
- 日本語・英語locale基盤と漫画編集workspace・Creator Chat・画像生成・書き出し・設定・更新・Hub連携の英語表示、dialog focus管理、reduced motion、Windows forced colorsに対応しています。製品Electronを使う日英18画面・状態のaxe A・AA監査は合計392項目合格・違反0件です。実質3要素の判定保留、main process messageの翻訳、画像生成状態の自動評価、Narrator確認は未完了です。
