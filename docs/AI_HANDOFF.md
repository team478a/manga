...
## 0.0 モニター利用条件の管理警告（2026-09-24）

- PR #520 merge commit `fe1ddf2ecf23f5d2a3eca52711d6030dfe698c94`から`codex/admin-monitor-operational-warnings-20260924`を作成した。
...
- Implementation commitは`5cfcdbe`。PR #503はmerge commit `3cff3e8`でマージ済みで、マージ後Required Quality run `35169748338`とDesktop Windows run `35169748296`も成功した。
...
- Production、Cloud、Provider、Runtime／model、生成、招待、配布、credit、実monitor・実承認・実proposal・実台帳操作なし。未追跡`apps/desktop/artifacts/`はcommit対象外。実運用は外部前提と対象付き承認を待つ。

---

## 0.0 Desktop Adult Stage 1初回招待台帳read-only監査（2026-09-17）

- BaseはPR #501 merge commit `e53e251`。マージ後Required Quality run `35165212620`とDesktop Windows run `35165212632`は成功済み。Branchは`codex/adult-stage1-ledger-audit-20260917`。
- Implementation commitは`a899361`、Draft PRは#502。全CI／Vercel Previewを確認中。
- Stage 1初回proposal、現在台帳、承認、消費receipt、候補assessment、backup、intent、適用receiptを変更せず検査し、未適用、適用準備、中断後receipt回復、適用完了を4状態で表示するCLIを追加した。
- 既存applyの承認／source／proposal／intent検証を共有し、receipt契約も全field、digest、時系列まで検証する。証跡欠損・矛盾・改変、監査中競合はfail closedで停止する。適用準備後の監査はintent準備時刻を境界にするため期限後も履歴確認できるが、新規適用は許可しない。
- candidate ID、monitor ID、pathを標準出力へ出さず、file変更、台帳適用、外部処理を行わない。監査結果は対象付き運用承認を代替しない。
...
## 0.0 Desktop Adult Pilot招待台帳状態遷移read-only監査（2026-09-17）

- BaseはPR #500 merge commit `91e09ef`。マージ後Required Quality run `35159053386`とDesktop Windows run `35159053390`は成功済み。Branchは`codex/adult-pilot-ledger-status-audit-20260917`。
- Implementation commitは`8e3a492`。PR #501はmerge commit `e53e251`でマージ済みで、マージ後Required Quality run `35165212620`とDesktop Windows run `35165212632`も成功した。
- 状態遷移proposal、現在台帳、backup、intent、receiptを変更せず検査し、未適用、適用準備、中断後receipt回復、適用完了を4状態で表示するCLIを追加した。
- 既存applyのsource／proposal／intent検証を共有し、receipt契約も全field、digest、時系列まで検証する。証跡欠損・矛盾・改変、監査中競合はfail closedで停止する。
- monitor IDとpathを標準出力へ出さず、file変更、状態適用、外部処理を行わない。監査結果は対象付き運用承認を代替しない。
...
- Production、Cloud、Provider、Runtime／model、生成、招待、配布、credit、実monitor、実proposal、実台帳操作なし。未追跡`apps/desktop/artifacts/`は未変更・未commit。実状態遷移proposal操作は対象付き運用承認を待つ。
...
- Implementation commitは`a9058ad`。PR #500はmerge commit `91e09ef`でマージ済みで、マージ後Required Quality run `35159053386`とDesktop Windows run `35159053390`も成功した。
...
- Implementation commitは`d5c0ae8`。PR #499はmerge commit `7a91a70`でマージ済みで、マージ後Required Quality run `35122591058`とDesktop Windows run `35122591027`も成功した。
...
- Production、Cloud、Provider、Runtime／model、生成、招待、配布、credit、実monitor、実proposal、実台帳操作なし。未追跡`apps/desktop/artifacts/`は未変更・未commit。実台帳反映はPR #500と対象付き運用承認を待つ。
...
- Implementation commitは`0ddee44`、Draft PRは#498。全CI／Vercel Previewを確認中。
...
- 集中16/16、Desktop 315/315、Hub 1008/1008、Canvas 26/26、AI 50/50、a11y 29画面blocking violation 0、deps error 0（既知warning 2件）、lint、typecheck、Hub／Desktop build、migration 83/83、RC構造、Prettier、diff check成功。PR #498の全CI／Vercel Previewを確認中。canonical readinessは5 READY／4 BLOCKEDでstrict失敗を維持する。
...
- Implementation commitは`970b803`、Draft PRは#497。全CI／Vercel Previewを確認中。
- 消費済みStage 1承認、固定receipt、候補assessment、承認時点の台帳を照合し、内容非保持`INVITED` entryを追加した別fileの台帳proposalを作るCLIを追加した。実台帳、配布、メール、Runtime／model、生成は変更しない。
- copy・改変、source変更、不正な日時、個人情報、重複monitor、進行中Stage 1、上書きを拒否する。candidate ID、monitor ID、pathは標準出力へ表示せず、candidate IDはproposalへ保存しない。
- 集中8/8、Desktop 307/307、Hub 1008/1008、Canvas 26/26、AI 50/50、a11y 29画面blocking violation 0、deps error 0（既知warning 2件）、lint、typecheck、Hub／Desktop build、migration 83/83、RC構造、diff check成功。PR #497の全CI／Vercel Previewを確認中。canonical readinessは5 READY／4 BLOCKEDでstrict失敗を維持する。
- Production、Cloud、Provider、Runtime／model、生成、招待、配布、credit、証明書、実候補者・実承認・実台帳操作なし。未追跡`apps/desktop/artifacts/`は未変更・未commit。
...
- Branchは`codex/adult-stage1-invitation-authorization-20260916`、実装commitは`bdb4b37`、Draft PRは#496。
...
- canonical readinessは5 READY／4 BLOCKEDでstrict失敗を維持する。Production、Cloud、Provider、Runtime／model、生成、招待、配布、credit、証明書、実候補者・実承認操作なし。未追跡`apps/desktop/artifacts/`は未変更・未commit。次はPR #496の正本同期commitをpushし、全CI／Vercel Preview確認で停止する。
...
- Commit `2fc3e46`をpushしてDraft PR #489を作成した。初回HEADでCore quality run `35066494718`（3分33秒）、Migration roundtrip（55秒）、Desktop Windows run `35066494767`（4分13秒）、Vercel Preview／Preview Commentsはすべて成功した。
- Branchは`codex/adult-stage0-fixed-bundle-evidence-20260916`。Production、Cloud、Provider、Runtime／model、生成、招待、配布、credit、証明書購入、実署名、Secret、Release変更なし。未追跡`apps/desktop/artifacts/`は未commit。次は正本同期commitの最終CI／Vercel Preview成功で停止する。実Bundle検証・取込・配布・Stage 0開始は行わない。
...
- 集中25/25、Hub 1002/1002、Hub型検査、lint、依存／module／size境界、migration 83/83、Hub build、RC構造、diff check成功。既知warning 2件と外部設定・手動E2EのPENDINGは維持する。
- Commit `1192dad`をpushしてDraft PR #479を作成した。最初のHEADでRequired Quality run `35034833343`、Desktop Windows run `35034833406`、Vercel Preview／Preview Commentsはすべて成功した。Previewは`https://vercel.com/team478as-projects/mangai-hub-staging/EoaLyH4ftZp4XnVEKj5fJVBkdhA5`。
- Production、DB、Provider、Job、Asset、Canvas、credit変更なし。Branchは`codex/cloud-panel-recovery-editor-focus-20260916`。次は正本同期commitの最終CI／Preview成功で停止し、実生成は別承認を待つ。
...
## 0.0 Cloud原稿編集「次に行う操作」導線修正（2026-09-15）
...
- Draft PR [#463](https://github.com/team478a/manga/pull/463)はDraft／MERGEABLE。HEAD `ebe84b6`のRequired Quality run `34846254710`、Desktop Windows run `34846254735`、Vercel、Preview Commentsはすべて成功し、[Preview](https://mangai-hub-staging-git-codex-docs-qua-7f7f5a-team478as-projects.vercel.app)はReadyである。
- 次は証跡同期後の最終HEADの全CI／Vercel Preview成功で停止する。merge後もProduction migration適用は別の実行時明示承認を待つ。
...
- Draft PR [#462](https://github.com/team478a/manga/pull/462)はDraft／MERGEABLE。実装HEAD `d59b286`のRequired Quality run `34844164187`、Migration roundtrip、Desktop Windows run `34844164109`、Vercel、Preview Commentsは成功した。[Preview](https://mangai-hub-staging-git-codex-feat-qua-90e2ec-team478as-projects.vercel.app)はReadyである。
- 次は証跡同期後の最終HEADで全CI／Vercel Preview成功を確認して停止する。merge後のProduction操作は責任者の実行時明示承認まで行わない。
...
- 設計の実装分割1として、第三者裁定のstrict schema、6状態の状態機械、遷移guard、匿名派生判定を追加した。既存Human response検証を再利用する。
- Primary A/B本人、完全一致ケース、Blind-first順序違反、独立判断なしの提出、確定後編集、不要裁定、重複有効裁定、棄権をfail closedで扱う。全必要ケース確定時だけPilot裁定完了とする。
- 裁定後も元agreement／kappa、`formalBenchmarkEligible=false`、自動採用なしを維持する。DB／API／UIは未接続で、Production、migration、回答、割当、通知、Provider、credit、生成操作なし。
- 集中7/7、Hub 968/968、Canvas 26/26、AI 50/50、Desktop 230/230、a11y 29画面0 violation、migration 81/81、deps、lint、typecheck、build、RC structure、diff check成功。次はDraft PRの全CI／Preview成功で停止し、merge後にDB実装を別PRで行う。
...
- Productionの完了済み`batch_private_01`を個人識別子・自由記述・回答時刻を取得しない`SELECT`だけで集計した。Primary A/Bは完全一致5/28件（17.86%）、verdict一致11/28件（39.29%）、Cohen's kappa 0.1250で、23件が第三者裁定対象である。
- Primary 56回答はgood 17／borderline 7／bad 32、平均confidence 3.9286。補助Panel 3名・84回答はgood 41／borderline 16／bad 27、平均confidence 4.0000だった。Panel票はPrimary判定へ加算していない。
- 判定は`needs_adjudication`で、完全一致90%・kappa 0.75の閾値はいずれも未達。正式Benchmark要件140画像・Primary独立回答280件に対して28画像・56件のため、`formalBenchmarkEligible=false`を維持する。
- Production、DB、migration、回答、割当、画像、通知、作品、Provider、Job、Asset、credit、生成操作なし。自動採用・削除もない。次はdocs-only Draft PRの全CI／Vercel Preview成功後、23件の第三者裁定手順を別工程で設計する。
...
- 責任者承認後、Productionへ`202609140002_cloud_storyboard_short_page_count.sql`を適用した。SQL Editorは成功し、`pg_constraint`のread-only照会でもネームの許可ページ数が4〜48件になったことを確認した。
- `test`の「雨宿りポスト」で4ページ初稿ネームを1回生成し、版`1bcc4b66-8bb3-48e9-bdd4-f47a6a0e5b78`として保存・再読込できた。Production DBの同一行も`page_count=4`だった。
- Provider request 1件を使用し、AI利用数は96/100から97/100。ネーム採用、修正版生成、画像生成、報告状態、通知、画像Provider、Job、Assetは変更していない。
- docs-only差分のRC構造検査とdiff checkは成功した。外部環境の既存PENDING項目に変更はない。
- 次は本記録のdocs-only Draft PRを全CI／Vercel Preview成功まで確認する。merge後、該当する利用者報告のread-only監査を行い、状態変更・利用者通知・更新情報掲載は責任者の実行時承認を待つ。

---

## 0.0 4ページ短編ネーム保存制約の修正（2026-09-14）

- PR #448 merge後のProduction受入れを`test`で行い、4ページAIネームは生成できたがDB保存に失敗した。AI／アプリschemaは4〜48ページ対応、DB制約は8〜48ページの旧仕様であり、4ページ結果を拒否していた。
- `cloud_story_storyboard_versions_result_check`を4〜48ページへ同期するmigration、4〜7ページデータを削除せずrollbackを停止するguard、canonical schema、checksum、回帰テストを追加した。
- 集中16/16、Hub 955/955、Canvas 26/26、AI 50/50、Desktop 230/230、a11y 29画面blocking violation 0、migration 81/81、deps／lint／全typecheck／Hub・Desktop build／RC structure／diff check成功。
- 受入れでProvider request 1件、AI利用数95/100→96/100。保存ネーム版は0件。新migration、報告状態、通知、作品、画像Provider、Job、Assetは変更していない。
...
- 次は証跡同期後の最終HEADで全CI／Vercel Preview成功を確認して停止する。Production migration適用と4ページ再受入れはmerge後かつ責任者の明示承認後に別工程で行う。
...
- 責任者の実行時承認後、Productionの品質確認進捗タスク`842cfaaa-02f6-42bc-b468-766cee7b5aff`と4／8ページ・購入者表記タスク`ce0ab559-face-4a6e-a26b-170bf184d81b`を、各`detected`との一致条件付きで`resolved`へ更新した。
- 元報告`0d267d39-5a1c-4c10-98be-fe3219d8de5a`と`2441f7f6-c948-4ea8-a2d6-f48c1dbea923`も公開状態`resolved`となり、修正内容を説明する`monitor_report_status`通知を各1件作成・検証した。
- 残る`170fc105-edbe-499c-b147-e8f12b7e3f1f`は画像品質への肯定的感想であり、`detected`のまま変更していない。Production作品、Provider、Job、Asset、credit、生成処理も変更していない。
- 次は本記録のdocs-only Draft PRを全CI／Vercel Preview成功まで確認する。
...
- read-only検証で成人向けguardの維持、誤った内側alias比較の消滅、外側行の完全修飾参照を確認した。
- `test`で既存4ページ初稿から「4ページ構成と登場人物は維持し、最終ページの読後感を少し明るく」と修正版を1回生成した。新しい版`a8e48a7e-e0df-4b3b-bb2c-6c8952d348de`として保存され、成功通知、4ページ維持、明るい結末への反映を確認した。
- AI利用数は94/100から95/100。追加Provider実行、報告状態更新、利用者連絡は行っていない。
- 未解決表示の報告5件のうち、シナリオ保存、4／8ページ表示、品質確認進捗は修正済みだが状態未更新。画像品質の1件は肯定的感想。実質的な次の調査対象は原稿編集の未生成画像／200件超の要修正表示である。
- 次は本記録のdocs-only Draft PRを全CI／Vercel Preview成功まで確認し、その後に原稿編集報告をread-only調査する。
...
- Production管理画面でBatch active、画像28、割当5、開始案内0/5、提出済み除外1、実送信対象4を再確認した。
- 責任者の外部送信承認後、未提出・未送信4名へ開始案内を1回送信した。画面の成功通知、4名の個別送信時刻、開始案内4/5、対象なしbuttonを確認した。失敗0件。
- 28/28最終送信済み1名へは送信していない。Provider、生成Job、Asset、credit、作品データは変更していない。
- 次はモニター回答を待ち、進捗と問い合わせを既存管理画面で確認する。再送は新たな未送信対象がある場合だけ行う。
