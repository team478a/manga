...
## Phase 2: 報告品質と利用者フォロー

- 画面パス、ブラウザー、表示領域、言語、タイムゾーン、オンライン状態を報告時に自動取得します。
- PNG、JPEG、WebPのスクリーンショットを任意添付できます。上限は5MBで、非公開Storageへ保存します。
- 本文、件名、利用環境からメールアドレス、電話番号、APIキー、Bearer tokenなどを保存前にマスクします。
- 画面URLは`app.mang-ai.com`のパスだけを保存し、queryとfragmentを除去します。
- 一般報告は10分5件、24時間30件をDBで上限とします。
- 受付、確認、対応中、対応済み、完了を利用者の履歴へ表示し、状態変更を通知します。
- 管理画面では直近7日、未完了、重大報告を集計し、10分間だけ有効な署名URLで添付を確認します。

導入時はPhase 1の後に`202608030002_cloud_monitor_operations_phase2.sql`を適用し、報告、管理確認、状態更新、利用者通知、連投拒否を順に確認します。
