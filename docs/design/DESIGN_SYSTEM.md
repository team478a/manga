...
作成日: 2026-07-26
状態: **案（未承認）**。本文書はトークン・コンポーネントの設計案であり、実装はデザイン方針承認後に画面単位で開始する。
...
1. Desktop（紫系アクセント`#6848d8`）とHub（緑系アクセント`#3F7D58`）のブランドカラーを統一するか、製品ごとに維持するか。
2. Hubにダークモードを追加するか（Desktopは既にダーク固定のため、両製品でテーマ切替の要否が異なる）。
3. Desktop側のCSS基盤をCSS変数のまま拡張するか、Hubと同様にTailwindへ移行するか（移行は大規模変更になるため、本フェーズでは非推奨として案に記載するのみ）。
4. フォントファミリーをMANGAIブランドとして統一するか（現状Desktop/Hubで別々の指定）。

これらは実装着手前に責任者の承認を得る。承認されるまで、本文書のトークン値は「案」として扱い、コード上のCSS変数/Tailwind設定を書き換えない。
