...
| 利用者 | 延長後の期限（JST） | モニターAI利用数 | 現行期間credit 使用／予約 |
| --- | --- | ---: | ---: |
| 山極 和彦 | 2026-10-24 07:58:51.541 | 0/30 | 0/0 |
| hiroro | 2026-10-24 07:58:33.448 | 6/30 | 0/0 |
| 加藤周星 | 2026-10-24 07:58:15.221 | 8/30 | 0/0 |
| 樋口 美子 | 2026-10-24 08:01:15.175 | 33/50 | 0/0 |
| 工藤一郎 | 2026-10-24 08:00:34.787 | 1/30 | 0/0 |
| 青木隆康 | 2026-10-24 08:00:54.968 | 7/50 | 0/0 |
| なっかん | 2026-10-24 08:02:00.586 | 13/50 | 4/0 |
| 松浦周平 | 2026-10-24 08:02:18.106 | 25/50 | 0/0 |

- 1 transaction内で実行前snapshotと実行後を比較し、`expires_at`と`updated_at`以外のmonitor項目、Cloud AI entitlement全項目、全usage period／credit行が不変であることを確認した。8行すべて`monitor_fields_unchanged=true`、`entitlement_unchanged=true`だった。
...
- 新migration `202609240001_cloud_general_monitor_expiry_extension`とrollback、schema正本、manifestを同期した。集中10/10、Hub 1028/1028、migration 85/85、全typecheck、全lint、deps error 0（既知warning 2件）、Hub Production build、RC Repository structure、`git diff --check`成功。
- Implementation commitは`f88a3390`。Draft PR [#522](https://github.com/team478a/manga/pull/522)はMERGEABLEで、Core quality、Migration roundtrip、Windows build、Vercel Preview、Preview Commentsがすべて成功した。
...
- 検証: 集中2/2、Hub 1013/1013、Hub typecheck、lint、deps error 0（既知warning 2件）、Hub Production build、SVG実レンダリング、`git diff --check`成功。
...
- 状態: `MERGED / CI_PASSED / REAL_RETENTION_PROPOSAL_NOT_CREATED / REAL_EVIDENCE_NOT_DELETED / REAL_STAGE0_NOT_RUN`
...
- Implementation commit: `0593e60`。PR #505はmerge commit `e18c99c`でマージ済み。マージ後Required Quality run `35176879178`とDesktop Windows run `35176879177`も成功した。
...
- 次: 専用の削除承認・中断回復apply・read-only監査を実装済み。実proposal作成・実削除は対象付き明示承認を待つ。
...
- 状態: `MERGED / CI_PASSED / REAL_LEDGER_NOT_CHANGED / REAL_PILOT_OPERATION_NOT_RUN`
...
- Implementation commit: `5cfcdbe`。PR #503はmerge commit `3cff3e8`でマージ済み。マージ後Required Quality run `35169748338`とDesktop Windows run `35169748296`も成功した。
...
- 次: 実ライフサイクル監査や実proposal操作は、外部前提と対象付き運用承認を得た個別操作として行う。
...
- Base: PR #501 merge commit `e53e251`。マージ後Required Quality run `35165212620`（Core quality、Migration roundtrip）とDesktop Windows run `35165212632`は成功済み。
- Branch: `codex/adult-stage1-ledger-audit-20260917`。Implementation commit `a899361`。Draft PR: #502。
- Stage 1初回招待台帳proposalと現在台帳、承認、消費receipt、候補assessment、固定backup、intent、適用receiptをread-onlyで検査し、`PROPOSAL_READY`、`APPLY_PREPARED`、`RECOVERY_REQUIRED`、`APPLIED`の4状態を判定する専用CLIを追加した。
- 既存applyの承認／source／proposal／intent検証を共有し、receiptの全field、digest、時系列も検証する。証跡欠損・矛盾・改変、適用前後のどちらでもない台帳、監査中の内容・存在状態変更をfail closedで拒否する。適用準備後はintentの準備時刻を承認境界として使い、期限後も中断・完了状態を監査できるが、新規適用は許可しない。
- CLIはfileを作成・更新・削除せず、candidate ID、monitor ID、pathを標準出力へ表示しない。監査成功はproposalレビュー、対象付き適用承認、再実行承認を代替せず、招待、配布、Runtime／model、生成、credit操作を行わない。
- 検証: 集中27/27、Desktop 353/353、Hub 1008/1008、Canvas 26/26、AI 50/50、Desktop a11y 29画面blocking violation 0、deps error 0（既知warning 2件）、lint、typecheck、Hub／Desktop Production build、migration 83/83、RC Repository structure READY、Prettier、`git diff --check`成功。Viteの既知chunk warning、外部設定・手動E2Eの既知PENDINGは不変である。
- Production、Cloud、Provider、Runtime／model、生成、招待、配布、credit、実monitor、実承認、実proposal、実台帳を変更していない。未追跡`apps/desktop/artifacts/`は未変更・未commitである。
- 次: 正本同期commitをpushし、PR #502の全CI／Vercel Preview成功まで確認する。実proposal監査・適用は行わない。
...
- Branch: `codex/adult-pilot-ledger-status-audit-20260917`。Implementation commit `8e3a492`。PR #501はmerge commit `e53e251`でマージ済み。マージ後Required Quality run `35165212620`とDesktop Windows run `35165212632`も成功した。
...
- 次: 実状態遷移proposalの監査と適用は、対象付き運用承認を得た個別操作として行う。
...
- 状態: `MERGED / CI_PASSED / REAL_PROPOSAL_NOT_CREATED / REAL_LEDGER_NOT_CHANGED / REAL_PILOT_OPERATION_NOT_RUN`
...
- Implementation commit: `d5c0ae8`。PR #499はmerge commit `7a91a70`でマージ済み。マージ後Required Quality run `35122591058`とDesktop Windows run `35122591027`は成功済み。
...
- 次: 状態遷移proposalの実台帳反映は、PR #500の専用applyと対象付き運用承認が整うまで行わない。

---

## 2026-09-16 Desktop Adult Stage 1招待台帳proposal適用

- 状態: `IMPLEMENTED / LOCAL_VALIDATION_PASSED / DRAFT_PR_OPEN / CI_PENDING / REAL_LEDGER_NOT_CHANGED / REAL_DISTRIBUTION_NOT_RUN / STAGE1_DISTRIBUTION_BLOCKED`
...
- Implementation commit: `0ddee44`。Draft PR: #498。
...
- 次: PR #498の全CI／Vercel Preview成功を確認して停止する。実proposal適用は外部前提完了と対象付き運用承認を待つ。
...
- 実装commit `6327983`をpushし、Draft PR #495を作成した。
- 次: 正本同期commitをpushし、全CI／Vercel Preview成功で停止する。実Stage 0開始、実機証跡回収・取込、Stage 1配布は外部前提と対象付き明示承認を待つ。
...
- Commit `d3024ff`をpushしてDraft PR #493を作成した。初回HEAD `3de5d9c`のCore quality run `35083872068`（3分22秒）、Migration roundtrip（58秒）、Desktop Windows run `35083872095`（3分32秒）、Vercel Preview／Preview Commentsはすべて成功した。Production、Cloud、Provider、Runtime／model、生成、招待、配布、credit、証明書購入、実署名、Secret、Release、実候補者・実証跡・実operation packageは変更していない。未追跡`apps/desktop/artifacts/`は未変更・未commitである。
- 次: この正本同期commitの全CI／Vercel Preview成功まで確認して停止する。実operation package作成、artifact送付、Stage 0開始は別の実行時承認を待つ。
...
- 次: この正本同期commitの最終CI／Vercel Preview成功で停止する。実Bundleの取得・検証・取込とStage 0開始は別の実行時承認を待つ。
...
- 証跡はartifact version、Stage 0専用目的、配布未許可、両署名状態、同一署名者判定、installer／製品EXE／blockmap／更新metadata／SBOM／checksumのSHA-256だけを保持する。絶対path、file名、証明書thumbprint／subject、PIN、秘密鍵、作品内容を保存せず、既存証跡を上書きしない。
...
- 責任者の実行時明示承認後、8 targetを1回だけ永続登録した。公式Workerを`mode=run`で1回実行したrun `34963035642`は`requests=3 / processed=3 / status=failed`で安全に終了した。
- Production read-only診断は、最新Batchが`active`、target 8件中pending 5件／failed 3件、失敗codeは`billing_unavailable`、generation Job link 0件だった。原因はAI entitlementが`trialing / admin`のまま2026-09-12に期限切れとなっていたことである。plan、料金行、通貨は有効だった。
...
- 責任者がProductionでPanel Reviewer D／Eへ各1件のcanary割り当てを明示承認後、管理画面から`case_000004`をPanel Reviewer Dへ、`case_000005`をPanel Reviewer Eへ1件ずつ割り当てた。各操作の完了表示は「不一致ケース1件へ裁定担当を割り当てました。案内は送信していません」である。
...
## 2026-09-15 Cloud原稿編集「次に行う操作」導線修正
...
- Draft PR [#467](https://github.com/team478a/manga/pull/467)はDraft／MERGEABLE。実装HEAD `58b7de7`のRequired Quality run `34916648485`（Core quality／Migration roundtrip）、Desktop Windows run `34916648430`、Vercel、Vercel Preview Commentsはすべて成功した。[Preview](https://mangai-hub-staging-1igxhtbx6-team478as-projects.vercel.app)はReadyである。
- 次: この証跡同期後の最終HEADで全CI／Vercel Preview成功を確認して停止する。merge後のProduction反映確認、対応完了通知、報告状態変更は別工程とする。
...
- 専用repositoryは裁定担当ID、理由、時刻を取得せず、downloadは管理者認証、`private, no-store`、`nosniff`とする。裁定完了後も元agreement／kappaを変更せず、`formalBenchmarkEligible=false`、自動採用・削除なしを維持する。
...
- 割当前に、対象が1ケースだけであること、担当者がPrimary A/B本人でないこと、この操作では外部送信・通知を行わないことの3確認を必須にした。担当停止は専用revoke RPCだけを通し、既存回答とappend-only監査履歴を削除しない。
...
- Production、DB、migration、回答、割当、画像、通知、作品、Provider、Job、Asset、credit、生成処理は変更していない。
- Draft PR [#455](https://github.com/team478a/manga/pull/455)はDraft／MERGEABLE。実装HEAD `522f348`のCore quality run `34819489512`、Migration roundtrip、Windows build run `34819489713`、Vercel、Vercel Preview Commentsはすべて成功した。
- 次: この証跡同期後の最終HEADで全CI／Vercel Preview成功を確認して停止する。merge後、Productionの匿名集計値をread-onlyで確認し、Primary不一致の第三者裁定手順を別工程で決める。
...
- AI／Zod契約は4〜48ページを許可している一方、Production DBの`cloud_story_storyboard_versions_result_check`だけが旧仕様の8〜48ページを要求していた。生成された4ページ結果がこの制約で拒否されたことが追加原因である。
