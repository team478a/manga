...
2026-08-29の後続実装で、完成済みページまたは画像配置済みコマの表示画像を候補採用で変更する場合、通常の品質4項目とは別に既存原稿への影響確認を必須にした。これは採用前のUI preflightであり、自動採用、自動不採用、Canvas保存、Provider実行、credit消費は行わない。
...
2026-08-29の対象コマ最終比較で、2ページ候補は既存の時計・焼損証拠画像を置き換える候補と判明した。現行画像も同じ意図を満たし、空コマや完成率を改善しないため見送る。19ページの夜間足場2件だけを空コマ向け再利用対象とする。

再利用候補2件を将来採用できた場合、未配置は144から142へ減る見込みだが、候補採用前の確定値にはしない。Pilot対象とcredit上限は候補処理後に再計算する。

これらはProduction書込みを含むため、実行時は対象作品・操作・回数を明示して責任者承認を得る。今回の監査では実行しない。

### 再利用候補2件の採用実行ランブック

1. Productionをread-onlyで開き、対象作品、19ページ、画像0/4、対象2コマが空、処理待ち・実行中Job 0、credit予約0を確認する。
2. 19ページの男女対峙・足場指示候補を開く。既存原稿変更警告がないことと品質4項目を確認し、明示承認の範囲内で1件だけ採用・保存する。
3. 再読込し、対象コマだけが更新され、他コマ、他ページ、元Asset履歴、creditが変化していないことを確認する。
4. 19ページの奥行き構図候補について同じ手順を繰り返す。1件目で異常があれば2件目へ進まない。
5. 最後に配置済み15/157、未配置142、19ページ画像2/4、処理待ち・実行中Job 0、credit予約0をread-onlyで確認する。

対象外は2ページ候補、重大不良4件、完成済み22ページの余剰4件である。採用作業ではProvider生成、Job登録、credit予約・消費、候補削除、自動修復を行わない。誤対象、既存画像変更警告、保存失敗、完成判定の予期しない変化があれば即時停止し、追加操作前に報告する。

2026-08-29、責任者の明示承認後に本ランブックを実行した。19ページの対象2件は品質4項目を確認して採用され、作品画面の確定値は配置15/157、未配置142、19ページ2/4、要修正273、更新番号59、生成中0ページとなった。2件目の操作中に保存競合表示が出たため追加上書きせず、別タブと作品画面のread-only表示で永続化を確認した。credit表示は使用4・予約0・残り16で不変であり、Provider生成、Job登録、credit予約・消費は行っていない。

## 残り142コマの生成計画

全142コマを一括投入しない。Provider、model、pricingを変更せず、既存のページbatch、checkpoint、credit reservation、fail-closed契約を使う。
...
1. Pilot対象を23–24ページ、1案／コマ、最大16 credit・最大予約`$1.44`、再実行なしとして承認するか。
2. 実行前に作品画風・人物設定を正本化し、本人セッションでモニターAI残りを再確認するか。

責任者の明示承認前に、Production候補採用・修復、Provider実行、credit予約は行わない。
