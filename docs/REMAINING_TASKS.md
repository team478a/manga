...
| 優先 | タスク                       | 現在の制限                                                                          |
| ---- | ---------------------------- | ----------------------------------------------------------------------------------- |
| P0   | ハイブリッド生成Phase 1基盤  | Router・作品ポリシー・route履歴・ローカル実行ゲート・Project内Asset Libraryまで完了 |
| P1   | safe素材JobとLibrary route   | 背景・小物・効果のJob入力UIと、検索一致時にAsset Libraryを選ぶ実行経路が未接続      |
