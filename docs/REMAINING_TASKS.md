...
HubとDesktopの主要機能、ローカル品質ゲート、Windowsインストール・製品版起動・アンインストールE2E、SBOM・checksum生成までは完了しています。残作業の中心は外部サービスを使うRC受入れ、コード署名、初回公開です。
...
| 優先 | タスク                         | 現在の制限                                      |
| ---- | ------------------------------ | ----------------------------------------------- |
| P1   | 別ドライブ保存先の削除検証     | `.trash`移動を別ドライブで未確認                |
| P1   | DesktopからHubへの更新操作     | 端末認証は現在`works:read`の読み取り専用        |
| P1   | Undo対象拡張                   | 素材削除、AI生成、書き出し、AI設定は対象外      |
