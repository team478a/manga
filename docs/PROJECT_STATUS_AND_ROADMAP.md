...
- 手動・自動Projectバックアップと復元は実装済み。履歴込み完全バックアップ、DB破損時リカバリー、クラッシュレポートは未実装
...
5. 履歴込み完全バックアップ、DB破損時のリカバリー（自動バックアップは実装済み）
