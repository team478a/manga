...
- 手動Projectバックアップ・復元は実装済み。自動バックアップ、履歴込み完全バックアップ、クラッシュレポートは未実装
...
5. 自動バックアップ、履歴込み完全バックアップ、DB破損時のリカバリー
