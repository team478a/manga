...
- 販売パッケージ経由の下書き作成と、公開済み作品のDesktop読み取り連携を実装済み。非公開下書き照会・更新は端末認可の設計後に対応
...
2026-07-14進捗: Phase 3のコード実装は完了しました。販売パッケージによる確認・下書き作成、公開情報の匿名照会に加え、Hubで本人が承認する端末コード方式を実装しました。承認済みDesktopは自分の非公開下書きと停止中商品を読み取り専用で確認できます。実Supabase環境での複数端末・期限切れ・失効E2Eとホスティング層のrate limit設定はPhase 4で実施します。
