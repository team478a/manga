...
import { CLOUD_ADULT_STORYBOARD_FEATURE_KEY } from "@/lib/cloud-adult-storyboard";
...
export async function setCloudAdultStoryboardGrantAction(
  profileId: string,
  formData: FormData,
) {
  const { profile: actor } = await requireAdmin();
  ...
  if (!parsed.success)
    redirect(`/admin/users/${encodeURIComponent(profileId)}?error=${encodeURIComponent("成人向けAIネームの許可設定を確認してください")}`);
  const { error } = await createAdminClient().rpc("set_cloud_adult_feature_grant", {
    p_actor_profile_id: actor.id,
    p_target_profile_id: parsed.data.profileId,
    p_feature_key: CLOUD_ADULT_STORYBOARD_FEATURE_KEY,
    p_status: parsed.data.status,
    p_source: parsed.data.source,
    p_valid_until: parsed.data.validUntil,
    p_admin_note: parsed.data.adminNote,
  ...
  if (error)
    redirect(`/admin/users/${parsed.data.profileId}?error=${encodeURIComponent("成人向けAIネームの許可を更新できませんでした")}`);
  revalidatePath(`/admin/users/${parsed.data.profileId}`);
  redirect(`/admin/users/${parsed.data.profileId}?message=${encodeURIComponent("成人向けAIネームの許可を更新しました")}`);
}
