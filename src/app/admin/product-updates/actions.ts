...
import {
  editProductUpdate,
  findRecentProductUpdates,
  insertProductUpdate,
  updateProductUpdateState,
...
export async function createProductUpdateAction(formData: FormData) {
  ...
  try {
    ...
    const duplicateResult = await findRecentProductUpdates(
      profile.id,
  ...
  try {
    const { error } = await insertProductUpdate({
      title: parsed.data.title,
      summary: parsed.data.summary,
      details: parsed.data.details || null,
      category: parsed.data.category,
      actionUrl: parsed.data.actionUrl || null,
      publishedAt: parsed.data.publishNow ? new Date().toISOString() : null,
...
export async function changeProductUpdateStateAction(formData: FormData) {
  ...
  try {
    const { error } = await updateProductUpdateState(
...
export async function editProductUpdateAction(formData: FormData) {
  ...
  let updateFailed = false;
  try {
    const result = await editProductUpdate({
