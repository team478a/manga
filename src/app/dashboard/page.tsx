...
import { FileArchive, ImagePlus, Images, PackagePlus, ReceiptText, ShoppingBag } from "lucide-react";
    ...
    <main className="page">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-lg text-stone-600">作品、販売商品、グッズ申請をここから管理します。</p>
        </div>
        <Link className="button" href="/dashboard/works/new">作品を投稿</Link>
      </div>
      ...
      <section className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {cards.map((card) => (
          <Link className="panel block transition hover:-translate-y-0.5 hover:border-leaf" href={card.href} key={card.title}>
            ...
            <p className="mt-3 text-lg leading-relaxed text-stone-600">{card.body}</p>
          </Link>
        ))}
      </section>
      <div className="mt-8 grid gap-5 lg:grid-cols-[1fr_1fr]">
        <form action={updateProfile} className="panel space-y-5">
          ...
          <div>
            ...
            <input className="field" id="displayName" name="displayName" defaultValue={profile.display_name} required />
          </div>
          <div>
            <label className="label" htmlFor="bio">自己紹介</label>
            <textarea className="field min-h-32" id="bio" name="bio" defaultValue={profile.bio ?? ""} />
          </div>
        ...
        <section className="panel">
          ...
          <Link className="button mt-5" href="/dashboard/works/new">作品をアップロード</Link>
        </section>
      </div>
    </main>
  );
