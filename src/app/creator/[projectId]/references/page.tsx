...
export default async function VisualReferencesPage({ params, searchParams }: {
  ...
  const [characters, world, referenceWorkspace] = await Promise.all([
  ...
  const panels = (await Promise.all(workspace.pages.map(async (page) => {
  ...
  return <main className="page">
    ...
    {!referenceWorkspace.available ? <section className="panel mt-6"><h2 className="text-xl font-bold">準備が必要です</h2><p className="mt-2">参照画像migrationを適用すると利用できます。</p></section> : <>
      <section className="panel mt-6">
        <h2 className="text-xl font-bold">参照画像を追加</h2>
        <form action={uploadVisualReferenceAction.bind(null, projectId)} className="mt-4 grid gap-4 md:grid-cols-2">
      ...
      <section className="panel mt-6">
        ...
        <form action={assignPanelSubjectAction.bind(null, projectId)} className="mt-4 grid gap-4 md:grid-cols-2">
