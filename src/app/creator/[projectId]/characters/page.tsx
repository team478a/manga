...
export default async function CharacterProfilesPage({ params, searchParams }: {
  ...
  return <main className="page">
    ...
    {!resultLoad.ok ? <CloudDataNotice className="mt-6">キャラクター設定を一時的に読み込めません。作品とページの編集は影響を受けません。</CloudDataNotice> : !result.available ? <section className="panel mt-6"><h2 className="text-xl font-bold">準備が必要です</h2><p className="mt-2">キャラクターProfile migrationを適用すると利用できます。</p></section> : <>
      <section className="panel mt-6"><h2 className="text-xl font-bold">新しいキャラクター</h2><p className="mt-2 text-sm text-stone-600">画像生成に使うには、年齢、体格、髪、衣装、変えてはいけない特徴をすべて入力してください。</p><CharacterForm projectId={projectId} /></section>
      <section className="mt-6 space-y-4" aria-label="保存済みキャラクター">
        {result.profiles.map((profile) => {
          const missingFields = getMissingCloudCharacterVisualFields(profile);
          return <details className="panel" key={profile.id}>
            ...
            {missingFields.length ? <p className="mt-3 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">画像生成の必須項目が不足しています：{missingFields.join("、")}</p> : <p className="mt-3 rounded-lg border border-green-200 bg-green-50 p-3 text-sm text-green-800">画像生成に必要な人物設定を保存済みです。</p>}
...
function CharacterForm({ projectId, profile }: { projectId: string; profile?: Awaited<ReturnType<typeof listCloudCharacterProfiles>>["profiles"][number] }) {
  return <form action={saveCharacterProfileAction.bind(null,projectId)} className="mt-5 grid gap-4 md:grid-cols-2">
    ...
    <label className="block"><span className="label">見た目の年齢（必須）</span><input className="field" name="appearanceAge" defaultValue={profile?.appearance_age} maxLength={120} placeholder="20代前半" required /></label>
    <label className="block"><span className="label">体格（必須）</span><input className="field" name="bodyBuild" defaultValue={profile?.body_build} maxLength={300} placeholder="小柄、細身" required /></label>
    <label className="block"><span className="label">髪型・髪色（必須）</span><textarea className="field min-h-20" name="hair" defaultValue={profile?.hair} maxLength={300} required /></label>
    <label className="block"><span className="label">基本衣装（必須）</span><textarea className="field min-h-20" name="costume" defaultValue={profile?.costume} maxLength={500} required /></label>
    ...
    <label className="block"><span className="label">変えてはいけない特徴（必須・1行1項目）</span><textarea className="field min-h-24" name="immutableTraits" defaultValue={profile?.immutable_traits.join("\n")} maxLength={1500} required /></label>
