import { z } from "zod";
import { getCloudAdultResearchAccess } from "./cloud-adult-research.ts";
import { DomainError, PermissionDeniedError } from "./domain-errors.ts";
import { createClient } from "./supabase/server.ts";

export const CLOUD_ADULT_AI_PLANNING_FEATURE_KEY = "adult_ai_planning";
export const CLOUD_ADULT_AI_PLANNING_TERMS_VERSION =
  "adult-ai-planning-v1";

export const cloudAdultAiPlanningFeatureEnabled = () =>
  process.env.CLOUD_ADULT_AI_PLANNING_ENABLED?.toLowerCase() === "true";

export type CloudAdultAiPlanningAccess = {
  allowed: boolean;
  reason:
    | "allowed"
    | "feature_disabled"
...
export async function getCloudAdultAiPlanningAccess(
  profileId: string,
): Promise<CloudAdultAiPlanningAccess> {
  if (!cloudAdultAiPlanningFeatureEnabled())
  ...
  const [settings, grant, consent] = await Promise.all([
    ...
    supabase
      ...
      .eq("feature_key", CLOUD_ADULT_AI_PLANNING_FEATURE_KEY)
      .maybeSingle<{ status: string; valid_until: string | null }>(),
    supabase
      .from("cloud_adult_ai_planning_consents")
      .select("terms_version,revoked_at")
      .eq("profile_id", profileId)
      .eq("terms_version", CLOUD_ADULT_AI_PLANNING_TERMS_VERSION)
      .is("revoked_at", null)
  ...
  if (grant.data.status !== "approved")
    return { allowed: false, reason: "grant_inactive" };
  if (
    grant.data.valid_until &&
    Date.parse(grant.data.valid_until) <= Date.now()
  )
...
export function assertCloudAdultAiPlanningAllowed(
  access: CloudAdultAiPlanningAccess,
) {
  if (!access.allowed)
    throw new PermissionDeniedError(
      access.reason === "consent_required"
        ? "成人向けAI企画の利用条件と外部AI送信への同意が必要です。"
        : "成人向けAI企画は現在利用できません。",
    );
}
...
export async function recordCloudAdultAiPlanningConsent(
  profileId: string,
  formData: FormData,
) {
    ...
    .upsert(
      {
        ...
        terms_version: CLOUD_ADULT_AI_PLANNING_TERMS_VERSION,
