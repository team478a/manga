...
import type { CloudPanelContinuityStateInput } from "@/lib/cloud-panel-continuity-state";

export async function getCloudVisualReferenceWorkspace(projectId: string) {
  const { supabase } = await cloudCreatorContext();
  await getCloudProjectWorkspace(projectId);
  const [references, assignments, bindings, states, policy, versions, continuityStates] = await Promise.all([
    supabase
    ...
    supabase.from("cloud_panel_continuity_states").select("id,page_id,panel_id,subject_kind,subject_id,time_of_day,weather,state_note,holding_hand,screen_side,gaze_direction,continues_from_panel_id").eq("project_id",projectId).order("updated_at",{ascending:false}),
  ]);
  if (references.error?.code === "42P01" || assignments.error?.code === "42P01")
    return {
      ...
      p1Available: false,characterVersions: [],bindings: [],stateAssignments: [],readinessPolicy: "block" as const,continuityAvailable:false,continuityStates:[],
    };
  ...
  const continuityAvailable=continuityStates.error?.code!=="42P01";
  if ((!p1Unavailable && (bindings.error || states.error || (policy.error && policy.error.code!=="PGRST116") || versions.error))||(continuityAvailable&&continuityStates.error))
    throw new DomainError("INTERNAL_ERROR","人物参照の版設定を読み込めませんでした。",{cause:bindings.error??states.error??policy.error??versions.error??continuityStates.error});
  ...
  return {
    ...
    continuityAvailable,continuityStates:continuityStates.data??[],
  };
}
...
export async function saveCloudPanelContinuityState(input:CloudPanelContinuityStateInput){const{supabase}=await cloudCreatorContext();const result=await supabase.rpc("save_cloud_panel_continuity_state",{p_project_id:input.projectId,p_page_id:input.pageId,p_panel_id:input.panelId,p_subject_kind:input.subjectKind,p_subject_id:input.subjectId,p_time_of_day:input.timeOfDay,p_weather:input.weather,p_state_note:input.stateNote,p_holding_hand:input.holdingHand,p_screen_side:input.screenSide,p_gaze_direction:input.gazeDirection,p_continues_from_panel_id:input.continuesFromPanelId});if(result.error)throw new DomainError("INTERNAL_ERROR","コマの連続状態を保存できませんでした。",{cause:result.error});}
export async function deleteCloudPanelContinuityState(projectId:string,stateId:string){const{supabase}=await cloudCreatorContext();const result=await supabase.rpc("delete_cloud_panel_continuity_state",{p_project_id:projectId,p_state_id:stateId});if(result.error)throw new DomainError("INTERNAL_ERROR","コマの連続状態を解除できませんでした。",{cause:result.error});}
