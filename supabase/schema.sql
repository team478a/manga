...
create table if not exists public.cloud_story_proposal_runs(id uuid primary key default gen_random_uuid(),owner_profile_id uuid not null references public.profiles(id) on delete cascade,research_report_id uuid not null references public.cloud_market_research_reports(id) on delete restrict,status text not null check(status='completed'),result jsonb not null check(jsonb_typeof(result)='object' and result->>'engineVersion'='proposal-rules-v1' and result->>'classification'='ai_inference' and result->>'containsGeneratedMarketNumbers'='false' and jsonb_typeof(result->'candidates')='array' and jsonb_array_length(result->'candidates')=3 and pg_column_size(result)<=131072),engine_version text not null check(engine_version='proposal-rules-v1'),completed_at timestamptz not null,created_at timestamptz not null default now());
create index if not exists cloud_story_proposal_runs_owner_idx on public.cloud_story_proposal_runs(owner_profile_id,created_at desc);create index if not exists cloud_story_proposal_runs_report_idx on public.cloud_story_proposal_runs(research_report_id,created_at desc);alter table public.cloud_story_proposal_runs enable row level security;grant select,insert on public.cloud_story_proposal_runs to authenticated;grant select,insert,delete on public.cloud_story_proposal_runs to service_role;drop policy if exists "cloud_story_proposal_runs_owner_read" on public.cloud_story_proposal_runs;create policy "cloud_story_proposal_runs_owner_read" on public.cloud_story_proposal_runs for select using(owner_profile_id=public.current_profile_id());drop policy if exists "cloud_story_proposal_runs_owner_insert" on public.cloud_story_proposal_runs;create policy "cloud_story_proposal_runs_owner_insert" on public.cloud_story_proposal_runs for insert with check(owner_profile_id=public.current_profile_id() and status='completed' and exists(select 1 from public.cloud_market_research_reports report where report.id=research_report_id and report.owner_profile_id=public.current_profile_id() and report.status='completed' and report.input->>'contentClass'='general'));
create table if not exists public.cloud_story_proposal_selections(id uuid primary key default gen_random_uuid(),owner_profile_id uuid not null references public.profiles(id) on delete cascade,research_report_id uuid not null references public.cloud_market_research_reports(id) on delete restrict,proposal_run_id uuid not null references public.cloud_story_proposal_runs(id) on delete restrict,candidate_id text not null check(candidate_id in('candidate-balanced','candidate-differentiated','candidate-focused')),candidate_snapshot jsonb not null check(jsonb_typeof(candidate_snapshot)='object' and candidate_snapshot->>'id'=candidate_id and pg_column_size(candidate_snapshot)<=32768),selected_at timestamptz not null default now(),unique(research_report_id));
create index if not exists cloud_story_proposal_selections_owner_idx on public.cloud_story_proposal_selections(owner_profile_id,selected_at desc);alter table public.cloud_story_proposal_selections enable row level security;grant select,insert on public.cloud_story_proposal_selections to authenticated;grant select,insert,delete on public.cloud_story_proposal_selections to service_role;drop policy if exists "cloud_story_proposal_selections_owner_read" on public.cloud_story_proposal_selections;create policy "cloud_story_proposal_selections_owner_read" on public.cloud_story_proposal_selections for select using(owner_profile_id=public.current_profile_id());drop policy if exists "cloud_story_proposal_selections_owner_insert" on public.cloud_story_proposal_selections;create policy "cloud_story_proposal_selections_owner_insert" on public.cloud_story_proposal_selections for insert with check(owner_profile_id=public.current_profile_id() and exists(select 1 from public.cloud_story_proposal_runs run where run.id=proposal_run_id and run.owner_profile_id=public.current_profile_id() and run.research_report_id=research_report_id and exists(select 1 from jsonb_array_elements(run.result->'candidates') candidate where candidate->>'id'=candidate_id and candidate=candidate_snapshot)));
create table if not exists public.cloud_scenario_runs(id uuid primary key default gen_random_uuid(),owner_profile_id uuid not null references public.profiles(id) on delete cascade,proposal_selection_id uuid not null references public.cloud_story_proposal_selections(id) on delete restrict,research_report_id uuid not null references public.cloud_market_research_reports(id) on delete restrict,parent_run_id uuid references public.cloud_scenario_runs(id) on delete restrict,revision_number integer not null check(revision_number between 1 and 1000),status text not null check(status='completed'),result jsonb not null check(jsonb_typeof(result)='object' and result->>'engineVersion'='scenario-rules-v1' and result->>'classification'='ai_inference' and(result->>'totalPages')::integer between 1 and 2000 and jsonb_typeof(result->'characters')='array' and jsonb_array_length(result->'characters')=3 and jsonb_typeof(result->'acts')='array' and jsonb_array_length(result->'acts')=3 and jsonb_typeof(result->'scenes')='array' and jsonb_array_length(result->'scenes') between 1 and 8 and pg_column_size(result)<=262144),engine_version text not null check(engine_version='scenario-rules-v1'),completed_at timestamptz not null,created_at timestamptz not null default now(),unique(proposal_selection_id,revision_number));
create index if not exists cloud_scenario_runs_owner_idx on public.cloud_scenario_runs(owner_profile_id,created_at desc);create index if not exists cloud_scenario_runs_selection_idx on public.cloud_scenario_runs(proposal_selection_id,revision_number desc);alter table public.cloud_scenario_runs enable row level security;grant select on public.cloud_scenario_runs to authenticated;grant select,insert,delete on public.cloud_scenario_runs to service_role;drop policy if exists "cloud_scenario_runs_owner_read" on public.cloud_scenario_runs;create policy "cloud_scenario_runs_owner_read" on public.cloud_scenario_runs for select using(owner_profile_id=public.current_profile_id());
create or replace function public.create_cloud_scenario_run(p_proposal_selection_id uuid,p_parent_run_id uuid,p_result jsonb,p_completed_at timestamptz) returns uuid language plpgsql security definer set search_path=public as $$ declare v_profile_id uuid:=public.current_profile_id();v_selection public.cloud_story_proposal_selections%rowtype;v_parent public.cloud_scenario_runs%rowtype;v_report public.cloud_market_research_reports%rowtype;v_revision integer;v_id uuid;begin if v_profile_id is null then raise exception 'cloud_scenario_auth_required';end if;select * into v_selection from public.cloud_story_proposal_selections where id=p_proposal_selection_id and owner_profile_id=v_profile_id;if not found then raise exception 'cloud_scenario_selection_not_found';end if;select * into v_report from public.cloud_market_research_reports where id=v_selection.research_report_id and owner_profile_id=v_profile_id and status='completed' and input->>'contentClass'='general';if not found then raise exception 'cloud_scenario_research_not_found';end if;if p_parent_run_id is not null then select * into v_parent from public.cloud_scenario_runs where id=p_parent_run_id and owner_profile_id=v_profile_id and proposal_selection_id=p_proposal_selection_id;if not found then raise exception 'cloud_scenario_parent_not_found';end if;end if;if p_result->'proposalTrace'->>'proposalSelectionId' is distinct from p_proposal_selection_id::text or p_result->'proposalTrace'->>'candidateId' is distinct from v_selection.candidate_id or p_result->'proposalTrace'->>'researchReportId' is distinct from v_selection.research_report_id::text or p_result->'proposalTrace'->'sourceUrls' is distinct from v_selection.candidate_snapshot->'sourceUrls' or(p_result->>'totalPages')::integer is distinct from(v_report.input->>'pageCount')::integer then raise exception 'cloud_scenario_trace_invalid';end if;perform pg_advisory_xact_lock(hashtextextended(p_proposal_selection_id::text,0));select coalesce(max(revision_number),0)+1 into v_revision from public.cloud_scenario_runs where proposal_selection_id=p_proposal_selection_id;insert into public.cloud_scenario_runs(owner_profile_id,proposal_selection_id,research_report_id,parent_run_id,revision_number,status,result,engine_version,completed_at) values(v_profile_id,p_proposal_selection_id,v_selection.research_report_id,p_parent_run_id,v_revision,'completed',p_result,'scenario-rules-v1',p_completed_at) returning id into v_id;return v_id;end $$;
revoke all on function public.create_cloud_scenario_run(uuid,uuid,jsonb,timestamptz) from public,anon;grant execute on function public.create_cloud_scenario_run(uuid,uuid,jsonb,timestamptz) to authenticated,service_role;
create table if not exists public.cloud_scenario_confirmations(id uuid primary key default gen_random_uuid(),owner_profile_id uuid not null references public.profiles(id) on delete cascade,proposal_selection_id uuid not null references public.cloud_story_proposal_selections(id) on delete restrict,scenario_run_id uuid not null references public.cloud_scenario_runs(id) on delete restrict,scenario_snapshot jsonb not null check(jsonb_typeof(scenario_snapshot)='object' and scenario_snapshot->>'engineVersion'='scenario-rules-v1' and pg_column_size(scenario_snapshot)<=262144),confirmed_at timestamptz not null default now(),unique(proposal_selection_id));
create index if not exists cloud_scenario_confirmations_owner_idx on public.cloud_scenario_confirmations(owner_profile_id,confirmed_at desc);alter table public.cloud_scenario_confirmations enable row level security;grant select,insert on public.cloud_scenario_confirmations to authenticated;grant select,insert,delete on public.cloud_scenario_confirmations to service_role;drop policy if exists "cloud_scenario_confirmations_owner_read" on public.cloud_scenario_confirmations;create policy "cloud_scenario_confirmations_owner_read" on public.cloud_scenario_confirmations for select using(owner_profile_id=public.current_profile_id());drop policy if exists "cloud_scenario_confirmations_owner_insert" on public.cloud_scenario_confirmations;create policy "cloud_scenario_confirmations_owner_insert" on public.cloud_scenario_confirmations for insert with check(owner_profile_id=public.current_profile_id() and exists(select 1 from public.cloud_scenario_runs run where run.id=scenario_run_id and run.owner_profile_id=public.current_profile_id() and run.proposal_selection_id=proposal_selection_id and run.result=scenario_snapshot));
create or replace function public.create_cloud_scenario_run(p_proposal_selection_id uuid,p_parent_run_id uuid,p_result jsonb,p_completed_at timestamptz) returns uuid language plpgsql security definer set search_path=public as $$ declare v_profile_id uuid:=public.current_profile_id();v_selection public.cloud_story_proposal_selections%rowtype;v_parent public.cloud_scenario_runs%rowtype;v_report public.cloud_market_research_reports%rowtype;v_revision integer;v_id uuid;begin if v_profile_id is null then raise exception 'cloud_scenario_auth_required';end if;select * into v_selection from public.cloud_story_proposal_selections where id=p_proposal_selection_id and owner_profile_id=v_profile_id;if not found then raise exception 'cloud_scenario_selection_not_found';end if;select * into v_report from public.cloud_market_research_reports where id=v_selection.research_report_id and owner_profile_id=v_profile_id and status='completed' and input->>'contentClass'='general';if not found then raise exception 'cloud_scenario_research_not_found';end if;perform pg_advisory_xact_lock(hashtextextended(p_proposal_selection_id::text,0));if exists(select 1 from public.cloud_scenario_confirmations where proposal_selection_id=p_proposal_selection_id) then raise exception 'cloud_scenario_already_confirmed';end if;select coalesce(max(revision_number),0)+1 into v_revision from public.cloud_scenario_runs where proposal_selection_id=p_proposal_selection_id;if v_revision=1 and(p_parent_run_id is not null or p_result->>'revisionFocus' is distinct from 'initial') then raise exception 'cloud_scenario_initial_invalid';end if;if v_revision>1 and(p_parent_run_id is null or p_result->>'revisionFocus' not in('pacing','character','clarity')) then raise exception 'cloud_scenario_revision_invalid';end if;if p_parent_run_id is not null then select * into v_parent from public.cloud_scenario_runs where id=p_parent_run_id and owner_profile_id=v_profile_id and proposal_selection_id=p_proposal_selection_id;if not found then raise exception 'cloud_scenario_parent_not_found';end if;end if;if p_result->'proposalTrace'->>'proposalSelectionId' is distinct from p_proposal_selection_id::text or p_result->'proposalTrace'->>'candidateId' is distinct from v_selection.candidate_id or p_result->'proposalTrace'->>'researchReportId' is distinct from v_selection.research_report_id::text or p_result->'proposalTrace'->'sourceUrls' is distinct from v_selection.candidate_snapshot->'sourceUrls' or(p_result->>'totalPages')::integer is distinct from(v_report.input->>'pageCount')::integer then raise exception 'cloud_scenario_trace_invalid';end if;insert into public.cloud_scenario_runs(owner_profile_id,proposal_selection_id,research_report_id,parent_run_id,revision_number,status,result,engine_version,completed_at) values(v_profile_id,p_proposal_selection_id,v_selection.research_report_id,p_parent_run_id,v_revision,'completed',p_result,'scenario-rules-v1',p_completed_at) returning id into v_id;return v_id;end $$;
create table if not exists public.cloud_manga_generations(id uuid primary key default gen_random_uuid(),owner_profile_id uuid not null references public.profiles(id) on delete cascade,scenario_confirmation_id uuid not null unique references public.cloud_scenario_confirmations(id) on delete restrict,scenario_run_id uuid not null references public.cloud_scenario_runs(id) on delete restrict,project_id uuid not null unique references public.cloud_projects(id) on delete restrict,status text not null check(status='completed'),result jsonb not null check(jsonb_typeof(result)='object' and result->>'engineVersion'='manga-layout-rules-v1' and result->>'classification'='ai_inference' and(result->>'totalPages')::integer between 1 and 200 and jsonb_typeof(result->'pages')='array' and jsonb_array_length(result->'pages')=(result->>'totalPages')::integer and pg_column_size(result)<=2097152),engine_version text not null check(engine_version='manga-layout-rules-v1'),completed_at timestamptz not null,created_at timestamptz not null default now());
create index if not exists cloud_manga_generations_owner_idx on public.cloud_manga_generations(owner_profile_id,created_at desc);alter table public.cloud_manga_generations enable row level security;grant select on public.cloud_manga_generations to authenticated;grant select,insert,delete on public.cloud_manga_generations to service_role;drop policy if exists "cloud_manga_generations_owner_read" on public.cloud_manga_generations;create policy "cloud_manga_generations_owner_read" on public.cloud_manga_generations for select using(owner_profile_id=public.current_profile_id());
create or replace function public.build_cloud_manga_panels(p_page_id uuid,p_layout_id text,p_created_at timestamptz) returns jsonb language plpgsql volatile set search_path=public as $$
declare v_rects jsonb;v_panels jsonb:='[]'::jsonb;v_rect jsonb;v_index integer:=0;
begin
case p_layout_id
when 'single' then v_rects:='[{"x":48,"y":72,"width":1504,"height":2256}]'::jsonb;
when 'top_one_bottom_two' then v_rects:='[{"x":48,"y":72,"width":1504,"height":1110},{"x":48,"y":1206,"width":740,"height":1122},{"x":812,"y":1206,"width":740,"height":1122}]'::jsonb;
when 'four_equal' then v_rects:='[{"x":48,"y":72,"width":740,"height":1116},{"x":812,"y":72,"width":740,"height":1116},{"x":48,"y":1212,"width":740,"height":1116},{"x":812,"y":1212,"width":740,"height":1116}]'::jsonb;
when 'six_equal' then v_rects:='[{"x":48,"y":72,"width":740,"height":744},{"x":812,"y":72,"width":740,"height":744},{"x":48,"y":840,"width":740,"height":744},{"x":812,"y":840,"width":740,"height":744},{"x":48,"y":1608,"width":740,"height":720},{"x":812,"y":1608,"width":740,"height":720}]'::jsonb;
else raise exception 'cloud_manga_layout_invalid';
end case;
for v_rect in select value from jsonb_array_elements(v_rects) loop v_panels:=v_panels||jsonb_build_array(jsonb_build_object('id',gen_random_uuid(),'pageId',p_page_id,'name','コマ'||(v_index+1),'x',(v_rect->>'x')::integer,'y',(v_rect->>'y')::integer,'width',(v_rect->>'width')::integer,'height',(v_rect->>'height')::integer,'rotation',0,'zIndex',v_index,'visible',true,'locked',false,'borderColor','#111111','borderWidth',4,'fillColor','#ffffff','shape','rectangle','slant',0,'imageAssetId',null,'imageFit','cover','imageOffsetX',0,'imageOffsetY',0,'imageScale',1,'imageRotation',0,'imageOpacity',1,'createdAt',p_created_at::text,'updatedAt',p_created_at::text));v_index:=v_index+1;end loop;
return v_panels;
end $$;
revoke all on function public.build_cloud_manga_panels(uuid,text,timestamptz) from public,anon,authenticated;
create or replace function public.create_cloud_manga_generation(p_scenario_confirmation_id uuid,p_result jsonb,p_completed_at timestamptz) returns table(generation_id uuid,project_id uuid,first_page_id uuid) language plpgsql security definer set search_path=public as $$ declare v_profile_id uuid:=public.current_profile_id();v_confirmation public.cloud_scenario_confirmations%rowtype;v_existing public.cloud_manga_generations%rowtype;v_generation_id uuid:=gen_random_uuid();v_project_id uuid:=gen_random_uuid();v_episode_id uuid:=gen_random_uuid();v_page_id uuid;v_page jsonb;v_page_number integer;v_layout_id text;begin if v_profile_id is null then raise exception 'cloud_manga_auth_required';end if;perform pg_advisory_xact_lock(hashtextextended(p_scenario_confirmation_id::text,0));select * into v_existing from public.cloud_manga_generations where scenario_confirmation_id=p_scenario_confirmation_id and owner_profile_id=v_profile_id;if found then generation_id:=v_existing.id;project_id:=v_existing.project_id;select id into first_page_id from public.cloud_pages where cloud_pages.project_id=v_existing.project_id and deleted_at is null order by page_number limit 1;return next;return;end if;select * into v_confirmation from public.cloud_scenario_confirmations where id=p_scenario_confirmation_id and owner_profile_id=v_profile_id;if not found then raise exception 'cloud_manga_confirmation_not_found';end if;if p_result->'scenarioTrace'->>'confirmationId' is distinct from p_scenario_confirmation_id::text or p_result->'scenarioTrace'->>'scenarioRunId' is distinct from v_confirmation.scenario_run_id::text or p_result->'scenarioTrace'->>'proposalSelectionId' is distinct from v_confirmation.proposal_selection_id::text or(p_result->>'totalPages')::integer is distinct from(v_confirmation.scenario_snapshot->>'totalPages')::integer or p_result->>'title' is distinct from v_confirmation.scenario_snapshot->>'title' or(p_result->>'generatedAt')::timestamptz is distinct from p_completed_at or p_result->'projectSettings' is distinct from '{"ageRating":"全年齢","readingDirection":"rtl","width":1600,"height":2400,"dpi":300}'::jsonb then raise exception 'cloud_manga_trace_invalid';end if;if(p_result->>'totalPages')::integer not between 1 and 200 or jsonb_typeof(p_result->'pages') is distinct from 'array' or jsonb_array_length(p_result->'pages')<>(p_result->>'totalPages')::integer then raise exception 'cloud_manga_page_count_invalid';end if;if exists(select 1 from jsonb_array_elements(p_result->'pages') with ordinality page(value,position) where(value->>'pageNumber')::integer<>position or value->>'layoutId' not in('single','top_one_bottom_two','four_equal','six_equal') or value->>'pageRole' not in('opening','development','turning_point','climax','resolution') or(value->>'panelCount')::integer is distinct from case value->>'layoutId' when 'single' then 1 when 'top_one_bottom_two' then 3 when 'four_equal' then 4 when 'six_equal' then 6 end or not exists(select 1 from jsonb_array_elements(v_confirmation.scenario_snapshot->'scenes') scene where scene->>'id'=value->>'sceneId' and scene->>'heading'=value->>'sceneHeading' and scene->>'summary'=value->>'sceneSummary' and position between(scene->>'pageStart')::integer and(scene->>'pageEnd')::integer)) then raise exception 'cloud_manga_page_plan_invalid';end if;insert into public.cloud_projects(id,owner_profile_id,source_surface,content_class,title,description,age_rating,reading_direction,width,height,dpi) values(v_project_id,v_profile_id,'cloud','general',p_result->>'title','確定シナリオから作成したマンガ下書き','全年齢','rtl',1600,2400,300);insert into public.cloud_episodes(id,project_id,title,order_index) values(v_episode_id,v_project_id,'第1話',0);for v_page in select value from jsonb_array_elements(p_result->'pages') order by(value->>'pageNumber')::integer loop v_page_id:=gen_random_uuid();v_page_number:=(v_page->>'pageNumber')::integer;v_layout_id:=v_page->>'layoutId';if v_page_number=1 then first_page_id:=v_page_id;end if;insert into public.cloud_pages(id,project_id,episode_id,page_number,order_index,width,height) values(v_page_id,v_project_id,v_episode_id,v_page_number,v_page_number-1,1600,2400);insert into public.cloud_canvas_snapshots(project_id,page_id,revision,canvas,created_by_profile_id) values(v_project_id,v_page_id,0,jsonb_build_object('schemaVersion',1,'pageId',v_page_id,'width',1600,'height',2400,'backgroundColor','#ffffff','panels',public.build_cloud_manga_panels(v_page_id,v_layout_id,p_completed_at),'panelLayers',jsonb_build_array(),'balloons',jsonb_build_array(),'textObjects',jsonb_build_array()),v_profile_id);end loop;insert into public.cloud_manga_generations(id,owner_profile_id,scenario_confirmation_id,scenario_run_id,project_id,status,result,engine_version,completed_at) values(v_generation_id,v_profile_id,p_scenario_confirmation_id,v_confirmation.scenario_run_id,v_project_id,'completed',p_result,'manga-layout-rules-v1',p_completed_at);insert into public.cloud_project_versions(project_id,revision,manifest,created_by_profile_id) values(v_project_id,0,jsonb_build_object('event','manga_draft_generated','generationId',v_generation_id,'scenarioConfirmationId',p_scenario_confirmation_id,'pageCount',p_result->>'totalPages'),v_profile_id);generation_id:=v_generation_id;project_id:=v_project_id;return next;end $$;
revoke all on function public.create_cloud_manga_generation(uuid,jsonb,timestamptz) from public,anon;grant execute on function public.create_cloud_manga_generation(uuid,jsonb,timestamptz) to authenticated,service_role;
create table if not exists public.cloud_work_management_states(id uuid primary key default gen_random_uuid(),owner_profile_id uuid not null references public.profiles(id) on delete cascade,project_id uuid not null unique references public.cloud_projects(id) on delete cascade,status text not null default 'draft' check(status in('draft','review_ready','approved')),expected_project_revision bigint,release_notes text not null default '' check(char_length(release_notes)<=5000),review_ready_at timestamptz,approved_at timestamptz,created_at timestamptz not null default now(),updated_at timestamptz not null default now(),check((status='draft' and expected_project_revision is null and approved_at is null)or(status='review_ready' and expected_project_revision is not null and review_ready_at is not null and approved_at is null)or(status='approved' and expected_project_revision is not null and review_ready_at is not null and approved_at is not null)));
create table if not exists public.cloud_work_page_reviews(id uuid primary key default gen_random_uuid(),owner_profile_id uuid not null references public.profiles(id) on delete cascade,project_id uuid not null references public.cloud_projects(id) on delete cascade,page_id uuid not null unique references public.cloud_pages(id) on delete cascade,page_revision bigint not null check(page_revision>=0),note text not null default '' check(char_length(note)<=500),reviewed_at timestamptz not null default now(),created_at timestamptz not null default now(),updated_at timestamptz not null default now(),unique(project_id,page_id));
create index if not exists cloud_work_management_owner_idx on public.cloud_work_management_states(owner_profile_id,updated_at desc);create index if not exists cloud_work_page_reviews_project_idx on public.cloud_work_page_reviews(owner_profile_id,project_id,reviewed_at desc);alter table public.cloud_work_management_states enable row level security;alter table public.cloud_work_page_reviews enable row level security;grant select on public.cloud_work_management_states,public.cloud_work_page_reviews to authenticated;grant select,insert,update,delete on public.cloud_work_management_states,public.cloud_work_page_reviews to service_role;drop policy if exists "cloud_work_management_owner_read" on public.cloud_work_management_states;create policy "cloud_work_management_owner_read" on public.cloud_work_management_states for select using(owner_profile_id=public.current_profile_id());drop policy if exists "cloud_work_page_reviews_owner_read" on public.cloud_work_page_reviews;create policy "cloud_work_page_reviews_owner_read" on public.cloud_work_page_reviews for select using(owner_profile_id=public.current_profile_id());
create or replace function public.reset_cloud_work_management_on_revision() returns trigger language plpgsql security definer set search_path=public as $$ begin if new.revision is distinct from old.revision then update public.cloud_work_management_states set status='draft',expected_project_revision=null,review_ready_at=null,approved_at=null,updated_at=now() where project_id=new.id;end if;return new;end $$;
drop trigger if exists cloud_projects_reset_work_management on public.cloud_projects;create trigger cloud_projects_reset_work_management after update of revision on public.cloud_projects for each row execute function public.reset_cloud_work_management_on_revision();
create or replace function public.set_cloud_work_page_review(p_project_id uuid,p_page_id uuid,p_reviewed boolean,p_note text default '') returns uuid language plpgsql security definer set search_path=public as $$ declare v_profile_id uuid:=public.current_profile_id();v_project public.cloud_projects%rowtype;v_page public.cloud_pages%rowtype;begin if v_profile_id is null or p_reviewed is null or char_length(coalesce(p_note,''))>500 then raise exception 'cloud_work_input_invalid';end if;select * into v_project from public.cloud_projects where id=p_project_id and owner_profile_id=v_profile_id and content_class='general' and deleted_at is null for update;if not found then raise exception 'cloud_work_project_not_found';end if;select * into v_page from public.cloud_pages where id=p_page_id and project_id=p_project_id and deleted_at is null;if not found then raise exception 'cloud_work_page_not_found';end if;if p_reviewed then insert into public.cloud_work_page_reviews(owner_profile_id,project_id,page_id,page_revision,note,reviewed_at,updated_at) values(v_profile_id,p_project_id,p_page_id,v_page.revision,trim(coalesce(p_note,'')),now(),now()) on conflict(page_id) do update set owner_profile_id=excluded.owner_profile_id,project_id=excluded.project_id,page_revision=excluded.page_revision,note=excluded.note,reviewed_at=excluded.reviewed_at,updated_at=excluded.updated_at;else delete from public.cloud_work_page_reviews where page_id=p_page_id and project_id=p_project_id and owner_profile_id=v_profile_id;end if;insert into public.cloud_work_management_states(owner_profile_id,project_id,status) values(v_profile_id,p_project_id,'draft') on conflict(project_id) do update set status='draft',expected_project_revision=null,review_ready_at=null,approved_at=null,updated_at=now();return p_page_id;end $$;
create or replace function public.set_cloud_work_management_status(p_project_id uuid,p_status text,p_release_notes text,p_expected_project_revision bigint) returns text language plpgsql security definer set search_path=public as $$ declare v_profile_id uuid:=public.current_profile_id();v_project public.cloud_projects%rowtype;v_state public.cloud_work_management_states%rowtype;v_page_count integer;begin if v_profile_id is null or p_status not in('draft','review_ready','approved') or p_expected_project_revision is null or char_length(coalesce(p_release_notes,''))>5000 then raise exception 'cloud_work_input_invalid';end if;select * into v_project from public.cloud_projects where id=p_project_id and owner_profile_id=v_profile_id and content_class='general' and deleted_at is null for update;if not found then raise exception 'cloud_work_project_not_found';end if;if v_project.revision<>p_expected_project_revision then raise exception 'cloud_work_revision_conflict';end if;insert into public.cloud_work_management_states(owner_profile_id,project_id,status) values(v_profile_id,p_project_id,'draft') on conflict(project_id) do nothing;select * into v_state from public.cloud_work_management_states where project_id=p_project_id and owner_profile_id=v_profile_id for update;if p_status='draft' then update public.cloud_work_management_states set status='draft',expected_project_revision=null,release_notes=trim(coalesce(p_release_notes,'')),review_ready_at=null,approved_at=null,updated_at=now() where project_id=p_project_id;return 'draft';end if;select count(*) into v_page_count from public.cloud_pages where project_id=p_project_id and deleted_at is null;if nullif(trim(v_project.title),'') is null or nullif(trim(v_project.description),'') is null or v_page_count not between 1 and 200 or v_project.cover_page_id is null or not exists(select 1 from public.cloud_pages where id=v_project.cover_page_id and project_id=p_project_id and deleted_at is null) or exists(select 1 from public.cloud_pages page where page.project_id=p_project_id and page.deleted_at is null and not exists(select 1 from public.cloud_canvas_snapshots snapshot where snapshot.page_id=page.id and snapshot.project_id=p_project_id)) or exists(select 1 from public.cloud_pages page where page.project_id=p_project_id and page.deleted_at is null and not exists(select 1 from public.cloud_work_page_reviews review where review.page_id=page.id and review.project_id=p_project_id and review.owner_profile_id=v_profile_id and review.page_revision=page.revision)) or exists(select 1 from public.cloud_generation_jobs where project_id=p_project_id and status in('queued','running')) then raise exception 'cloud_work_not_ready';end if;if p_status='approved' and(v_state.status<>'review_ready' or v_state.expected_project_revision<>v_project.revision) then raise exception 'cloud_work_status_transition_invalid';end if;update public.cloud_work_management_states set status=p_status,expected_project_revision=v_project.revision,release_notes=trim(coalesce(p_release_notes,'')),review_ready_at=case when p_status='review_ready' then now() else review_ready_at end,approved_at=case when p_status='approved' then now() else null end,updated_at=now() where project_id=p_project_id;return p_status;end $$;
revoke all on function public.reset_cloud_work_management_on_revision() from public,anon,authenticated;revoke all on function public.set_cloud_work_page_review(uuid,uuid,boolean,text) from public,anon;revoke all on function public.set_cloud_work_management_status(uuid,text,text,bigint) from public,anon;grant execute on function public.set_cloud_work_page_review(uuid,uuid,boolean,text) to authenticated,service_role;grant execute on function public.set_cloud_work_management_status(uuid,text,text,bigint) to authenticated,service_role;
create table if not exists public.cloud_sales_preparations(id uuid primary key default gen_random_uuid(),owner_profile_id uuid not null references public.profiles(id) on delete cascade,project_id uuid not null unique references public.cloud_projects(id) on delete cascade,project_revision bigint not null check(project_revision>=0),work_id uuid not null unique references public.works(id),product_id uuid not null unique references public.digital_products(id),price integer not null check(price between 0 and 1000000),cover_url text not null check(char_length(cover_url) between 1 and 4096),product_path text not null check(char_length(product_path) between 1 and 4096),synced_at timestamptz not null default now(),updated_at timestamptz not null default now());
create index if not exists cloud_sales_preparations_owner_idx on public.cloud_sales_preparations(owner_profile_id,synced_at desc);alter table public.cloud_sales_preparations enable row level security;grant select on public.cloud_sales_preparations to authenticated;grant select,insert,update,delete on public.cloud_sales_preparations to service_role;drop policy if exists "cloud_sales_preparations_owner_read" on public.cloud_sales_preparations;create policy "cloud_sales_preparations_owner_read" on public.cloud_sales_preparations for select using(owner_profile_id=public.current_profile_id());
create or replace function public.sync_cloud_sales_preparation(p_project_id uuid,p_expected_revision bigint,p_cover_url text,p_product_path text,p_price integer,p_sales_description text) returns table(work_id uuid,product_id uuid) language plpgsql security definer set search_path=public as $$ declare v_profile_id uuid:=public.current_profile_id();v_project public.cloud_projects%rowtype;v_approval public.cloud_work_management_states%rowtype;v_synced record;begin if v_profile_id is null then raise exception 'cloud_sales_auth_required';end if;perform pg_advisory_xact_lock(hashtextextended(p_project_id::text,1));select * into v_project from public.cloud_projects where id=p_project_id and owner_profile_id=v_profile_id and content_class='general' and deleted_at is null for update;if not found then raise exception 'cloud_sales_project_not_found';end if;if v_project.revision<>p_expected_revision then raise exception 'cloud_sales_revision_conflict';end if;select * into v_approval from public.cloud_work_management_states where project_id=p_project_id and owner_profile_id=v_profile_id for update;if not found or v_approval.status<>'approved' or v_approval.expected_project_revision<>v_project.revision then raise exception 'cloud_sales_approval_required';end if;select * into v_synced from public.sync_cloud_marketplace_draft(p_project_id,p_expected_revision,p_cover_url,p_product_path,p_price,p_sales_description);if v_synced.work_id is null or v_synced.product_id is null then raise exception 'cloud_sales_sync_failed';end if;insert into public.cloud_sales_preparations(owner_profile_id,project_id,project_revision,work_id,product_id,price,cover_url,product_path,synced_at,updated_at) values(v_profile_id,p_project_id,v_project.revision,v_synced.work_id,v_synced.product_id,p_price,p_cover_url,p_product_path,now(),now()) on conflict(project_id) do update set owner_profile_id=excluded.owner_profile_id,project_revision=excluded.project_revision,work_id=excluded.work_id,product_id=excluded.product_id,price=excluded.price,cover_url=excluded.cover_url,product_path=excluded.product_path,synced_at=excluded.synced_at,updated_at=excluded.updated_at;return query select v_synced.work_id::uuid,v_synced.product_id::uuid;end $$;
revoke execute on function public.sync_cloud_marketplace_draft(uuid,bigint,text,text,integer,text) from authenticated;revoke all on function public.sync_cloud_sales_preparation(uuid,bigint,text,text,integer,text) from public,anon;grant execute on function public.sync_cloud_sales_preparation(uuid,bigint,text,text,integer,text) to authenticated,service_role;
