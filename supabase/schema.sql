...
create index if not exists cloud_adult_grok_audit_created_idx on public.cloud_adult_grok_audit_logs(created_at desc);
...
drop policy if exists "cloud_adult_grok_settings_admin_read" on public.cloud_adult_grok_settings;
create policy "cloud_adult_grok_settings_admin_read" on public.cloud_adult_grok_settings for select using(public.is_admin());
drop policy if exists "cloud_adult_grok_audit_admin_read" on public.cloud_adult_grok_audit_logs;
create policy "cloud_adult_grok_audit_admin_read" on public.cloud_adult_grok_audit_logs for select using(public.is_admin());
create or replace function public.set_cloud_adult_grok_provider(p_actor_profile_id uuid,p_api_key text,p_model text,p_enabled boolean) returns void language plpgsql security definer set search_path=public,vault as $$
declare v_settings public.cloud_adult_grok_settings%rowtype;v_secret_id uuid;v_action text;v_api_key text:=nullif(btrim(coalesce(p_api_key,'')),'');
begin
  if auth.role()<>'service_role' or not exists(select 1 from public.profiles where id=p_actor_profile_id and role='admin') then raise exception 'cloud_adult_grok_admin_required';end if;
  if p_model not in('grok-4.5','grok-4.20') then raise exception 'cloud_adult_grok_model_invalid';end if;
  if v_api_key is not null and(char_length(v_api_key)<20 or char_length(v_api_key)>500 or v_api_key~'\s') then raise exception 'cloud_adult_grok_key_invalid';end if;
  select * into v_settings from public.cloud_adult_grok_settings where singleton=true for update;v_secret_id:=v_settings.secret_id;
  if v_api_key is not null then
    if v_secret_id is null then v_secret_id:=vault.create_secret(v_api_key,'mangai_cloud_adult_xai','MANGAI Cloud adult text xAI API key');v_action:='configure';
    else perform vault.update_secret(v_secret_id,v_api_key,'mangai_cloud_adult_xai','MANGAI Cloud adult text xAI API key');v_action:='replace_key';end if;
  elsif p_enabled and v_secret_id is null then raise exception 'cloud_adult_grok_key_required';
  else v_action:=case when p_enabled then 'enable' else 'disable' end;end if;
  update public.cloud_adult_grok_settings set enabled=p_enabled,model=p_model,secret_id=v_secret_id,updated_by_profile_id=p_actor_profile_id,updated_at=now() where singleton=true;
  insert into public.cloud_adult_grok_audit_logs(actor_profile_id,action,model,enabled) values(p_actor_profile_id,v_action,p_model,p_enabled);
end;$$;
create or replace function public.get_cloud_adult_grok_runtime_config() returns table(enabled boolean,model text,api_key text) language plpgsql security definer set search_path=public,vault as $$
begin
  if auth.role()<>'service_role' then raise exception 'cloud_adult_grok_service_role_required';end if;
  return query select settings.enabled,settings.model,secrets.decrypted_secret from public.cloud_adult_grok_settings settings left join vault.decrypted_secrets secrets on secrets.id=settings.secret_id where settings.singleton=true;
end;$$;
revoke all on function public.set_cloud_adult_grok_provider(uuid,text,text,boolean) from public,anon,authenticated;
revoke all on function public.get_cloud_adult_grok_runtime_config() from public,anon,authenticated;
grant execute on function public.set_cloud_adult_grok_provider(uuid,text,text,boolean) to service_role;
grant execute on function public.get_cloud_adult_grok_runtime_config() to service_role;
