...
create table public.cloud_manga_panel_specifications(generation_job_id uuid primary key references public.cloud_generation_jobs(id)on delete cascade,owner_profile_id uuid not null references public.profiles(id)on delete restrict,project_id uuid not null references public.cloud_projects(id)on delete cascade,panel_id uuid not null,specification jsonb not null,created_at timestamptz not null default now(),check((specification->>'version')::integer=1));
...
create index cloud_manga_quality_evaluations_panel_rank_idx on public.cloud_manga_quality_evaluations(project_id,panel_id,overall_score desc,created_at);
