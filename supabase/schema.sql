...
do $$ begin if to_regprocedure('vault.create_secret(text,text,text)') is null then execute 'create extension if not exists supabase_vault with schema vault';end if;end $$;
create table if not exists public.cloud_general_monitor_email_settings(singleton boolean primary key default true check(singleton),enabled boolean not null default false,from_email text not null default '' check(char_length(from_email)<=254),from_name text not null default 'MANGAI運営' check(char_length(from_name) between 1 and 80),secret_id uuid,updated_by_profile_id uuid references public.profiles(id) on delete set null,updated_at timestamptz not null default now());
...
revoke all on function public.set_cloud_general_monitor_email_provider(uuid,text,text,text,boolean) from public,anon,authenticated;revoke all on function public.get_cloud_general_monitor_email_runtime_config() from public,anon,authenticated;grant execute on function public.set_cloud_general_monitor_email_provider(uuid,text,text,text,boolean) to service_role;grant execute on function public.get_cloud_general_monitor_email_runtime_config() to service_role;
