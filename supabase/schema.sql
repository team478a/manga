...
declare
  v_profile_id uuid:=public.current_profile_id();v_storyboard public.cloud_story_storyboard_versions%rowtype;v_existing public.cloud_story_storyboard_projects%rowtype;v_episode_id uuid;v_project_id uuid;v_page_id uuid;v_first_page_id uuid;v_page jsonb;v_page_index integer:=0;v_content_class text;v_revision bigint;
begin
  ...
  if v_page_index<>jsonb_array_length(v_storyboard.result->'pages') or v_page_index<>(v_storyboard.result->>'pageCount')::integer then raise exception 'storyboard_page_count_mismatch';end if;
  ...
  insert into public.cloud_project_versions(project_id,revision,manifest,created_by_profile_id) values(v_project_id,v_revision,jsonb_build_object('event','storyboard_materialized','storyboardVersionId',v_storyboard.id,'pageCount',v_page_index),v_profile_id);
  insert into public.cloud_story_storyboard_projects(owner_profile_id,storyboard_version_id,project_id,first_page_id) values(v_profile_id,v_storyboard.id,v_project_id,v_first_page_id);
