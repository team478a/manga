...
$$;
create table public.cloud_adult_ai_planning_settings(singleton boolean primary key default true check(singleton),enabled boolean not null default false,updated_by_profile_id uuid references public.profiles(id) on delete set null,updated_at timestamptz not null default now());
insert into public.cloud_adult_ai_planning_settings(singleton,enabled) values(true,false);
...
alter table public.cloud_story_proposal_runs add column content_class text not null default 'general' check(content_class in('general','adult'));
alter table public.cloud_story_proposal_selections add column content_class text not null default 'general' check(content_class in('general','adult'));
