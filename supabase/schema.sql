...
execution_phase text check(execution_phase is null or execution_phase in('queued','preparing','generating','validating','succeeded','failed','canceled','unknown')),failure_stage text check(failure_stage is null or failure_stage in('request','visual_readiness','moderation','quota','claim','lease','reference_resolution','provider','validation','storage','completion','quality','adoption','dialogue')),retry_disposition text check(retry_disposition is null or retry_disposition in('automatic','manual','none')),http_status integer check(http_status is null or http_status between 100 and 599),parent_job_id uuid references public.cloud_generation_jobs(id) on delete set null,root_job_id uuid references public.cloud_generation_jobs(id) on delete set null,workflow_version text check(workflow_version is null or char_length(workflow_version) between 1 and 200),seed text check(seed is null or char_length(seed) between 1 and 200),last_checkpoint_at timestamptz,
unique(created_by_profile_id,idempotency_key),unique(id,project_id),check(parent_job_id is null or parent_job_id<>id),check(root_job_id is null or root_job_id<>id),foreign key(page_id,project_id) references public.cloud_pages(id,project_id) on delete cascade);
...
create index if not exists cloud_generation_jobs_parent_idx on public.cloud_generation_jobs(parent_job_id) where parent_job_id is not null;
create index if not exists cloud_generation_jobs_root_idx on public.cloud_generation_jobs(root_job_id) where root_job_id is not null;
create table if not exists public.cloud_generation_job_events(id uuid primary key default gen_random_uuid(),job_id uuid not null references public.cloud_generation_jobs(id) on delete cascade,project_id uuid not null references public.cloud_projects(id) on delete cascade,owner_profile_id uuid not null references public.profiles(id) on delete cascade,execution_phase text not null check(execution_phase in('queued','preparing','generating','validating','succeeded','failed','canceled','unknown')),event_type text not null check(event_type in('phase_changed','retry_scheduled','lease_reclaimed','canceled','completed','failed')),attempt_number integer not null check(attempt_number between 0 and 100),metadata jsonb not null default'{}'::jsonb check(jsonb_typeof(metadata)='object'and pg_column_size(metadata)<=16384 and not metadata ?| array['prompt','apiKey','api_key','authorization','signedUrl','signed_url','responseBody','response_body']),created_at timestamptz not null default now(),foreign key(job_id,project_id) references public.cloud_generation_jobs(id,project_id) on delete cascade);
create index if not exists cloud_generation_job_events_job_idx on public.cloud_generation_job_events(job_id,created_at);
create index if not exists cloud_generation_job_events_project_idx on public.cloud_generation_job_events(project_id,created_at desc);
alter table public.cloud_generation_job_events enable row level security;
grant select on public.cloud_generation_job_events to authenticated;grant select,insert on public.cloud_generation_job_events to service_role;
drop policy if exists "cloud_generation_job_events_read" on public.cloud_generation_job_events;create policy "cloud_generation_job_events_read" on public.cloud_generation_job_events for select using(owner_profile_id=public.current_profile_id()and public.cloud_project_can_edit(project_id));
create or replace function public.link_cloud_generation_retry(p_source_job_id uuid,p_retry_job_id uuid)returns uuid language plpgsql security definer set search_path=public as $$declare v_profile uuid:=public.current_profile_id();v_source public.cloud_generation_jobs%rowtype;v_retry public.cloud_generation_jobs%rowtype;v_root uuid;begin if v_profile is null or p_source_job_id=p_retry_job_id then raise exception'cloud_generation_retry_lineage_invalid';end if;select*into v_source from public.cloud_generation_jobs where id=p_source_job_id and created_by_profile_id=v_profile for update;if not found then raise exception'cloud_generation_retry_lineage_invalid';end if;select*into v_retry from public.cloud_generation_jobs where id=p_retry_job_id and created_by_profile_id=v_profile for update;if not found or v_source.status<>'failed'or v_retry.status<>'queued'or v_source.project_id<>v_retry.project_id or v_source.page_id is distinct from v_retry.page_id or v_retry.parent_job_id is not null or v_retry.root_job_id is not null or not public.cloud_project_can_edit(v_source.project_id)then raise exception'cloud_generation_retry_lineage_invalid';end if;v_root:=coalesce(v_source.root_job_id,v_source.id);if v_root=p_retry_job_id then raise exception'cloud_generation_retry_lineage_cycle';end if;update public.cloud_generation_jobs set retry_disposition='manual',updated_at=now()where id=v_source.id;update public.cloud_generation_jobs set parent_job_id=v_source.id,root_job_id=v_root,execution_phase='queued',last_checkpoint_at=now(),updated_at=now()where id=v_retry.id;insert into public.cloud_generation_job_events(job_id,project_id,owner_profile_id,execution_phase,event_type,attempt_number,metadata)values(v_source.id,v_source.project_id,v_profile,'failed','retry_scheduled',greatest(v_source.attempt_count,0),jsonb_build_object('retryJobId',v_retry.id));return v_retry.id;end$$;
revoke all on function public.link_cloud_generation_retry(uuid,uuid)from public,anon;grant execute on function public.link_cloud_generation_retry(uuid,uuid)to authenticated,service_role;
...
create or replace function public.extend_cloud_general_monitor_expiry(p_actor_profile_id uuid,p_target_profile_id uuid,p_expires_at timestamptz,p_admin_note text) returns table(changed boolean,result_expires_at timestamptz) language plpgsql security definer set search_path=public as $$declare v_before public.cloud_general_monitor_enrollments%rowtype;v_after public.cloud_general_monitor_enrollments%rowtype;begin if auth.role()<>'service_role' or not exists(select 1 from public.profiles where id=p_actor_profile_id and role='admin') then raise exception 'cloud_general_monitor_admin_required';end if;if p_expires_at is null or p_expires_at<=now() then raise exception 'cloud_general_monitor_expiry_invalid';end if;if char_length(trim(coalesce(p_admin_note,''))) not between 1 and 500 then raise exception 'cloud_general_monitor_admin_note_invalid';end if;select * into v_before from public.cloud_general_monitor_enrollments where profile_id=p_target_profile_id for update;if not found then raise exception 'cloud_general_monitor_not_found';end if;if v_before.status<>'active' then raise exception 'cloud_general_monitor_not_active';end if;if p_expires_at<=v_before.starts_at then raise exception 'cloud_general_monitor_expiry_invalid';end if;if p_expires_at<v_before.expires_at then raise exception 'cloud_general_monitor_expiry_must_not_shorten';end if;if p_expires_at=v_before.expires_at then changed:=false;result_expires_at:=v_before.expires_at;return next;return;end if;update public.cloud_general_monitor_enrollments set expires_at=p_expires_at,updated_at=now() where profile_id=p_target_profile_id returning * into v_after;insert into public.cloud_general_monitor_audit_logs(actor_profile_id,target_profile_id,action,before_value,after_value) values(p_actor_profile_id,p_target_profile_id,'extend_expiry',to_jsonb(v_before),to_jsonb(v_after)||jsonb_build_object('operation_note',trim(p_admin_note)));changed:=true;result_expires_at:=v_after.expires_at;return next;end;$$;
revoke all on function public.extend_cloud_general_monitor_expiry(uuid,uuid,timestamptz,text) from public,anon,authenticated;grant execute on function public.extend_cloud_general_monitor_expiry(uuid,uuid,timestamptz,text) to service_role;
create or replace function public.record_cloud_general_monitor_expiry_email_sent(p_actor_profile_id uuid,p_target_profile_id uuid,p_expires_at timestamptz) returns boolean language plpgsql security definer set search_path=public as $$declare v_enrollment public.cloud_general_monitor_enrollments%rowtype;begin if auth.role()<>'service_role' or not exists(select 1 from public.profiles where id=p_actor_profile_id and role='admin') then raise exception 'cloud_general_monitor_admin_required';end if;select * into v_enrollment from public.cloud_general_monitor_enrollments where profile_id=p_target_profile_id for update;if not found then raise exception 'cloud_general_monitor_not_found';end if;if v_enrollment.status<>'active' or v_enrollment.expires_at<>p_expires_at then raise exception 'cloud_general_monitor_expiry_changed';end if;if exists(select 1 from public.cloud_general_monitor_audit_logs where target_profile_id=p_target_profile_id and action='expiry_extension_email_sent' and (after_value->>'expires_at')::timestamptz=p_expires_at) then return false;end if;insert into public.cloud_general_monitor_audit_logs(actor_profile_id,target_profile_id,action,before_value,after_value) values(p_actor_profile_id,p_target_profile_id,'expiry_extension_email_sent',null,jsonb_build_object('expires_at',p_expires_at));return true;end;$$;
revoke all on function public.record_cloud_general_monitor_expiry_email_sent(uuid,uuid,timestamptz) from public,anon,authenticated;grant execute on function public.record_cloud_general_monitor_expiry_email_sent(uuid,uuid,timestamptz) to service_role;
...
alter table public.cloud_general_monitor_feedback add column if not exists client_context jsonb not null default '{}'::jsonb,add column if not exists attachment_path text,add column if not exists attachment_paths text[] not null default '{}'::text[],add column if not exists public_status text not null default 'submitted',add column if not exists status_updated_at timestamptz not null default now();
update public.cloud_general_monitor_feedback set attachment_paths=array[attachment_path] where attachment_path is not null and cardinality(attachment_paths)=0;
alter table public.cloud_general_monitor_feedback drop constraint if exists cloud_general_monitor_feedback_attachment_paths_check;alter table public.cloud_general_monitor_feedback add constraint cloud_general_monitor_feedback_attachment_paths_check check(cardinality(attachment_paths) between 0 and 5 and octet_length(array_to_string(attachment_paths,''))<=2500);
...
alter table public.cloud_project_generation_readiness_policies enable row level security;grant select on public.cloud_project_generation_readiness_policies to authenticated;grant select,insert,update,delete on public.cloud_project_generation_readiness_policies to service_role;
drop policy if exists "cloud_project_generation_readiness_policies_owner_read" on public.cloud_project_generation_readiness_policies;create policy "cloud_project_generation_readiness_policies_owner_read" on public.cloud_project_generation_readiness_policies for select using(owner_profile_id=public.current_profile_id());
create or replace function public.save_cloud_project_generation_readiness_policy(p_project_id uuid,p_major_character_reference_policy text)returns void language plpgsql security definer set search_path=public,pg_temp as $$declare v_owner uuid:=public.current_profile_id();begin if v_owner is null or p_major_character_reference_policy not in('warn','block')or not exists(select 1 from public.cloud_projects p where p.id=p_project_id and p.owner_profile_id=v_owner)then raise exception 'cloud_generation_readiness_policy_invalid';end if;insert into public.cloud_project_generation_readiness_policies(project_id,owner_profile_id,major_character_reference_policy)values(p_project_id,v_owner,p_major_character_reference_policy)on conflict(project_id)do update set major_character_reference_policy=excluded.major_character_reference_policy,updated_at=now()where cloud_project_generation_readiness_policies.owner_profile_id=v_owner;end$$;
revoke all on function public.save_cloud_project_generation_readiness_policy(uuid,text)from public,anon;grant execute on function public.save_cloud_project_generation_readiness_policy(uuid,text)to authenticated,service_role;

create table if not exists public.cloud_character_state_assignments(id uuid primary key default gen_random_uuid(),project_id uuid not null references public.cloud_projects(id)on delete cascade,owner_profile_id uuid not null references public.profiles(id)on delete cascade,character_profile_id uuid not null,character_version_id uuid not null references public.cloud_character_profile_versions(id)on delete cascade,start_page integer not null check(start_page>0),end_page integer not null check(end_page>=start_page),scene_key text not null default''check(char_length(scene_key)<=120),assignment_label text not null check(char_length(trim(assignment_label))between 1 and 120),costume_override text not null default''check(char_length(costume_override)<=500),state_note text not null default''check(char_length(state_note)<=500),priority integer not null default 0 check(priority between 0 and 100),created_at timestamptz not null default now(),updated_at timestamptz not null default now(),foreign key(character_profile_id,project_id)references public.cloud_character_profiles(id,project_id)on delete cascade);
create index if not exists cloud_character_state_assignments_range_idx on public.cloud_character_state_assignments(character_version_id,start_page,end_page,priority desc);alter table public.cloud_character_state_assignments enable row level security;grant select on public.cloud_character_state_assignments to authenticated;grant select,insert,update,delete on public.cloud_character_state_assignments to service_role;drop policy if exists "cloud_character_state_assignments_owner_read" on public.cloud_character_state_assignments;create policy "cloud_character_state_assignments_owner_read" on public.cloud_character_state_assignments for select using(owner_profile_id=public.current_profile_id());
create or replace function public.save_cloud_character_state_assignment(p_project_id uuid,p_character_profile_id uuid,p_character_version_id uuid,p_start_page integer,p_end_page integer,p_scene_key text,p_assignment_label text,p_costume_override text,p_state_note text,p_priority integer default 0)returns uuid language plpgsql security definer set search_path=public,pg_temp as $$declare v_owner uuid:=public.current_profile_id();v_id uuid;begin if v_owner is null or p_start_page<1 or p_end_page<p_start_page or p_priority not between 0 and 100 or char_length(trim(coalesce(p_assignment_label,'')))not between 1 and 120 then raise exception 'cloud_character_state_assignment_invalid';end if;perform pg_advisory_xact_lock(hashtextextended(p_character_version_id::text,0));if not exists(select 1 from public.cloud_character_profile_versions v join public.cloud_character_profiles p on p.id=v.profile_id and p.project_id=v.project_id where v.id=p_character_version_id and v.profile_id=p_character_profile_id and v.project_id=p_project_id and v.owner_profile_id=v_owner and p.owner_profile_id=v_owner and p.deleted_at is null)or exists(select 1 from public.cloud_character_state_assignments a where a.character_version_id=p_character_version_id and int4range(a.start_page,a.end_page,'[]')&&int4range(p_start_page,p_end_page,'[]'))then raise exception 'cloud_character_state_assignment_invalid';end if;insert into public.cloud_character_state_assignments(project_id,owner_profile_id,character_profile_id,character_version_id,start_page,end_page,scene_key,assignment_label,costume_override,state_note,priority)values(p_project_id,v_owner,p_character_profile_id,p_character_version_id,p_start_page,p_end_page,trim(coalesce(p_scene_key,'')),trim(p_assignment_label),trim(coalesce(p_costume_override,'')),trim(coalesce(p_state_note,'')),p_priority)returning id into v_id;return v_id;end$$;
create or replace function public.delete_cloud_character_state_assignment(p_project_id uuid,p_assignment_id uuid)returns void language plpgsql security definer set search_path=public,pg_temp as $$begin delete from public.cloud_character_state_assignments where id=p_assignment_id and project_id=p_project_id and owner_profile_id=public.current_profile_id();if not found then raise exception 'cloud_character_state_assignment_not_found';end if;end$$;revoke all on function public.save_cloud_character_state_assignment(uuid,uuid,uuid,integer,integer,text,text,text,text,integer),public.delete_cloud_character_state_assignment(uuid,uuid)from public,anon;grant execute on function public.save_cloud_character_state_assignment(uuid,uuid,uuid,integer,integer,text,text,text,text,integer),public.delete_cloud_character_state_assignment(uuid,uuid)to authenticated,service_role;

create table if not exists public.cloud_panel_continuity_states(id uuid primary key default gen_random_uuid(),project_id uuid not null references public.cloud_projects(id)on delete cascade,owner_profile_id uuid not null references public.profiles(id)on delete cascade,page_id uuid not null,panel_id uuid not null,subject_kind text not null check(subject_kind in('character','location','prop')),subject_id uuid not null,time_of_day text not null default''check(char_length(time_of_day)<=80),weather text not null default''check(char_length(weather)<=80),state_note text not null default''check(char_length(state_note)<=500),holding_hand text not null default'unspecified'check(holding_hand in('unspecified','left','right','both','none')),screen_side text not null default'unspecified'check(screen_side in('unspecified','left','center','right')),gaze_direction text not null default''check(char_length(gaze_direction)<=120),continues_from_panel_id uuid,created_at timestamptz not null default now(),updated_at timestamptz not null default now(),foreign key(page_id,project_id)references public.cloud_pages(id,project_id)on delete cascade,unique(project_id,page_id,panel_id,subject_kind,subject_id));
create index if not exists cloud_panel_continuity_states_panel_idx on public.cloud_panel_continuity_states(project_id,page_id,panel_id);alter table public.cloud_panel_continuity_states enable row level security;grant select on public.cloud_panel_continuity_states to authenticated;grant select,insert,update,delete on public.cloud_panel_continuity_states to service_role;drop policy if exists "cloud_panel_continuity_states_owner_read" on public.cloud_panel_continuity_states;create policy "cloud_panel_continuity_states_owner_read" on public.cloud_panel_continuity_states for select using(owner_profile_id=public.current_profile_id());
create or replace function public.save_cloud_panel_continuity_state(p_project_id uuid,p_page_id uuid,p_panel_id uuid,p_subject_kind text,p_subject_id uuid,p_time_of_day text,p_weather text,p_state_note text,p_holding_hand text,p_screen_side text,p_gaze_direction text,p_continues_from_panel_id uuid default null)returns uuid language plpgsql security definer set search_path=public,pg_temp as $$declare v_owner uuid:=public.current_profile_id();v_id uuid;begin if v_owner is null or p_subject_kind not in('character','location','prop')or p_holding_hand not in('unspecified','left','right','both','none')or p_screen_side not in('unspecified','left','center','right')or not exists(select 1 from public.cloud_pages p join public.cloud_canvas_snapshots s on s.project_id=p.project_id and s.page_id=p.id and s.revision=p.revision cross join lateral jsonb_array_elements(coalesce(s.canvas->'panels','[]'::jsonb)) panel where p.id=p_page_id and p.project_id=p_project_id and p.deleted_at is null and panel->>'id'=p_panel_id::text)or not public.cloud_visual_subject_exists(p_project_id,p_subject_kind,p_subject_id)or(p_continues_from_panel_id is not null and(p_continues_from_panel_id=p_panel_id or not exists(select 1 from public.cloud_pages p join public.cloud_canvas_snapshots s on s.project_id=p.project_id and s.page_id=p.id and s.revision=p.revision cross join lateral jsonb_array_elements(coalesce(s.canvas->'panels','[]'::jsonb)) panel where p.project_id=p_project_id and p.deleted_at is null and panel->>'id'=p_continues_from_panel_id::text)))then raise exception 'cloud_panel_continuity_state_invalid';end if;if not exists(select 1 from public.cloud_projects p where p.id=p_project_id and p.owner_profile_id=v_owner)then raise exception 'cloud_panel_continuity_state_invalid';end if;insert into public.cloud_panel_continuity_states(project_id,owner_profile_id,page_id,panel_id,subject_kind,subject_id,time_of_day,weather,state_note,holding_hand,screen_side,gaze_direction,continues_from_panel_id)values(p_project_id,v_owner,p_page_id,p_panel_id,p_subject_kind,p_subject_id,trim(coalesce(p_time_of_day,'')),trim(coalesce(p_weather,'')),trim(coalesce(p_state_note,'')),p_holding_hand,p_screen_side,trim(coalesce(p_gaze_direction,'')),p_continues_from_panel_id)on conflict(project_id,page_id,panel_id,subject_kind,subject_id)do update set time_of_day=excluded.time_of_day,weather=excluded.weather,state_note=excluded.state_note,holding_hand=excluded.holding_hand,screen_side=excluded.screen_side,gaze_direction=excluded.gaze_direction,continues_from_panel_id=excluded.continues_from_panel_id,updated_at=now()where cloud_panel_continuity_states.owner_profile_id=v_owner returning id into v_id;return v_id;end$$;
create or replace function public.delete_cloud_panel_continuity_state(p_project_id uuid,p_state_id uuid)returns void language plpgsql security definer set search_path=public,pg_temp as $$begin delete from public.cloud_panel_continuity_states where id=p_state_id and project_id=p_project_id and owner_profile_id=public.current_profile_id();if not found then raise exception 'cloud_panel_continuity_state_not_found';end if;end$$;revoke all on function public.save_cloud_panel_continuity_state(uuid,uuid,uuid,text,uuid,text,text,text,text,text,text,uuid),public.delete_cloud_panel_continuity_state(uuid,uuid)from public,anon;grant execute on function public.save_cloud_panel_continuity_state(uuid,uuid,uuid,text,uuid,text,text,text,text,text,text,uuid),public.delete_cloud_panel_continuity_state(uuid,uuid)to authenticated,service_role;

create table if not exists public.cloud_panel_designs(id uuid primary key default gen_random_uuid(),project_id uuid not null references public.cloud_projects(id)on delete cascade,owner_profile_id uuid not null references public.profiles(id)on delete cascade,page_id uuid not null,panel_id uuid not null,revision bigint not null default 1 check(revision>0),design jsonb not null check(jsonb_typeof(design)='object'and design->>'schemaVersion'='1'and jsonb_typeof(design->'orderIndex')='number'and pg_column_size(design)<=65536),created_at timestamptz not null default now(),updated_at timestamptz not null default now(),foreign key(page_id,project_id)references public.cloud_pages(id,project_id)on delete cascade,unique(project_id,page_id,panel_id));
create table if not exists public.cloud_panel_design_versions(id uuid primary key default gen_random_uuid(),panel_design_id uuid not null references public.cloud_panel_designs(id)on delete cascade,project_id uuid not null references public.cloud_projects(id)on delete cascade,owner_profile_id uuid not null references public.profiles(id)on delete cascade,page_id uuid not null,panel_id uuid not null,revision bigint not null check(revision>0),design jsonb not null,created_at timestamptz not null default now(),unique(panel_design_id,revision));
create index if not exists cloud_panel_designs_page_order_idx on public.cloud_panel_designs(project_id,page_id,((design->>'orderIndex')::integer));create index if not exists cloud_panel_design_versions_design_idx on public.cloud_panel_design_versions(panel_design_id,revision desc);alter table public.cloud_panel_designs enable row level security;alter table public.cloud_panel_design_versions enable row level security;grant select on public.cloud_panel_designs,public.cloud_panel_design_versions to authenticated;grant select,insert,update,delete on public.cloud_panel_designs,public.cloud_panel_design_versions to service_role;drop policy if exists "cloud_panel_designs_owner_read" on public.cloud_panel_designs;create policy "cloud_panel_designs_owner_read" on public.cloud_panel_designs for select using(owner_profile_id=public.current_profile_id());drop policy if exists "cloud_panel_design_versions_owner_read" on public.cloud_panel_design_versions;create policy "cloud_panel_design_versions_owner_read" on public.cloud_panel_design_versions for select using(owner_profile_id=public.current_profile_id());
create or replace function public.save_cloud_panel_design(p_project_id uuid,p_page_id uuid,p_panel_id uuid,p_expected_revision bigint,p_design jsonb)returns table(panel_design_id uuid,revision bigint)language plpgsql security definer set search_path=public,pg_temp as $$declare v_owner uuid:=public.current_profile_id();v_existing public.cloud_panel_designs%rowtype;begin if v_owner is null or jsonb_typeof(p_design)<>'object'or p_design->>'schemaVersion'<>'1'or jsonb_typeof(p_design->'orderIndex')<>'number'or(p_design->>'orderIndex')::integer not between 0 and 499 or pg_column_size(p_design)>65536 or not exists(select 1 from public.cloud_projects where id=p_project_id and owner_profile_id=v_owner)or not exists(select 1 from public.cloud_pages p join public.cloud_canvas_snapshots s on s.project_id=p.project_id and s.page_id=p.id and s.revision=p.revision cross join lateral jsonb_array_elements(coalesce(s.canvas->'panels','[]'::jsonb)) panel where p.id=p_page_id and p.project_id=p_project_id and p.deleted_at is null and panel->>'id'=p_panel_id::text)then raise exception'cloud_panel_design_invalid';end if;select*into v_existing from public.cloud_panel_designs where project_id=p_project_id and page_id=p_page_id and panel_id=p_panel_id for update;if found then if p_expected_revision is distinct from v_existing.revision then raise exception'cloud_panel_design_revision_conflict';end if;update public.cloud_panel_designs set design=p_design,revision=cloud_panel_designs.revision+1,updated_at=now()where id=v_existing.id returning id,cloud_panel_designs.revision into panel_design_id,revision;else if p_expected_revision is not null then raise exception'cloud_panel_design_revision_conflict';end if;insert into public.cloud_panel_designs(project_id,owner_profile_id,page_id,panel_id,design)values(p_project_id,v_owner,p_page_id,p_panel_id,p_design)returning id,cloud_panel_designs.revision into panel_design_id,revision;end if;insert into public.cloud_panel_design_versions(panel_design_id,project_id,owner_profile_id,page_id,panel_id,revision,design)values(panel_design_id,p_project_id,v_owner,p_page_id,p_panel_id,revision,p_design);return next;end$$;revoke all on function public.save_cloud_panel_design(uuid,uuid,uuid,bigint,jsonb)from public,anon;grant execute on function public.save_cloud_panel_design(uuid,uuid,uuid,bigint,jsonb)to authenticated,service_role;
...
create index if not exists cloud_generation_run_checkpoints_batch_idx on public.cloud_generation_run_checkpoints(batch_id,created_at);
...
create or replace function public.record_cloud_generation_run_checkpoint(p_job_id uuid) returns uuid language plpgsql security definer set search_path=public as $$declare v_job public.cloud_generation_jobs%rowtype;v_target record;v_asset public.cloud_assets%rowtype;v_id uuid;begin if auth.role()<>'service_role' then raise exception 'cloud_worker_not_authorized';end if;select * into v_job from public.cloud_generation_jobs where id=p_job_id and status='completed' and output_asset_id is not null;if not found then raise exception 'cloud_generation_checkpoint_job_invalid';end if;select * into v_target from public.cloud_generation_batch_targets where generation_job_id=v_job.id;if not found then return null;end if;select * into v_asset from public.cloud_assets where id=v_job.output_asset_id and project_id=v_job.project_id and deleted_at is null;if not found or v_asset.sha256 is null then raise exception 'cloud_generation_checkpoint_asset_invalid';end if;insert into public.cloud_generation_run_checkpoints(target_id,batch_id,project_id,page_id,panel_id,source_page_revision,job_id,output_asset_id,output_sha256) values(v_target.id,v_target.batch_id,v_target.project_id,v_target.page_id,v_target.panel_id,v_target.source_page_revision,v_job.id,v_asset.id,v_asset.sha256) on conflict(target_id) do update set target_id=excluded.target_id where cloud_generation_run_checkpoints.job_id=excluded.job_id and cloud_generation_run_checkpoints.output_asset_id=excluded.output_asset_id and cloud_generation_run_checkpoints.output_sha256=excluded.output_sha256 returning id into v_id;if v_id is null then raise exception 'cloud_generation_checkpoint_conflict';end if;update public.cloud_generation_jobs set last_checkpoint_at=now(),updated_at=now() where id=v_job.id;return v_id;end$$;
...
create table if not exists public.cloud_character_reference_bindings(id uuid primary key default gen_random_uuid(),project_id uuid not null references public.cloud_projects(id) on delete cascade,owner_profile_id uuid not null references public.profiles(id) on delete cascade,character_profile_id uuid not null,character_version_id uuid not null references public.cloud_character_profile_versions(id) on delete cascade,asset_id uuid not null references public.cloud_assets(id) on delete restrict,reference_role text not null check(reference_role in('front','side','back','face','full_body','expression','costume_detail')),expression_key text not null default '' check(char_length(expression_key)<=80),priority integer not null default 0 check(priority between 0 and 100),review_status text not null default 'draft' check(review_status in('draft','approved','rejected')),created_at timestamptz not null default now(),updated_at timestamptz not null default now(),foreign key(character_profile_id,project_id) references public.cloud_character_profiles(id,project_id) on delete cascade,check((reference_role='expression' and char_length(expression_key)>=1)or(reference_role<>'expression' and expression_key='')),unique(character_version_id,asset_id,reference_role,expression_key));
create index if not exists cloud_character_reference_bindings_version_idx on public.cloud_character_reference_bindings(character_version_id,review_status,priority desc,created_at);
alter table public.cloud_character_reference_bindings enable row level security;grant select on public.cloud_character_reference_bindings to authenticated;grant select,insert,update,delete on public.cloud_character_reference_bindings to service_role;
drop policy if exists "cloud_character_reference_bindings_owner_read" on public.cloud_character_reference_bindings;create policy "cloud_character_reference_bindings_owner_read" on public.cloud_character_reference_bindings for select using(owner_profile_id=public.current_profile_id());
create or replace function public.save_cloud_character_reference_binding(p_project_id uuid,p_character_profile_id uuid,p_character_version_id uuid,p_asset_id uuid,p_reference_role text,p_expression_key text default null,p_priority integer default 0,p_review_status text default 'draft')returns uuid language plpgsql security definer set search_path=public,pg_temp as $$declare v_owner uuid:=public.current_profile_id();v_id uuid;begin if v_owner is null or p_reference_role not in('front','side','back','face','full_body','expression','costume_detail')or p_priority not between 0 and 100 or p_review_status not in('draft','approved','rejected')or((p_reference_role='expression')is distinct from(nullif(trim(coalesce(p_expression_key,'')),'')is not null))then raise exception 'cloud_character_reference_binding_invalid';end if;if not exists(select 1 from public.cloud_character_profile_versions v join public.cloud_character_profiles p on p.id=v.profile_id and p.project_id=v.project_id where v.id=p_character_version_id and v.profile_id=p_character_profile_id and v.project_id=p_project_id and v.owner_profile_id=v_owner and p.owner_profile_id=v_owner and p.deleted_at is null)or not exists(select 1 from public.cloud_assets a where a.id=p_asset_id and a.project_id=p_project_id and a.owner_profile_id=v_owner and a.deleted_at is null and a.mime_type like'image/%')then raise exception 'cloud_character_reference_binding_invalid';end if;insert into public.cloud_character_reference_bindings(project_id,owner_profile_id,character_profile_id,character_version_id,asset_id,reference_role,expression_key,priority,review_status)values(p_project_id,v_owner,p_character_profile_id,p_character_version_id,p_asset_id,p_reference_role,case when p_reference_role='expression'then trim(p_expression_key)else''end,p_priority,p_review_status)on conflict(character_version_id,asset_id,reference_role,expression_key)do update set priority=excluded.priority,review_status=excluded.review_status,updated_at=now()returning id into v_id;return v_id;end$$;
...
revoke all on function public.save_cloud_character_reference_binding(uuid,uuid,uuid,uuid,text,text,integer,text)from public,anon;revoke all on function public.delete_cloud_character_reference_binding(uuid,uuid)from public,anon;grant execute on function public.save_cloud_character_reference_binding(uuid,uuid,uuid,uuid,text,text,integer,text)to authenticated,service_role;grant execute on function public.delete_cloud_character_reference_binding(uuid,uuid)to authenticated,service_role;

create table if not exists public.cloud_manga_inspection_runs(id uuid primary key default gen_random_uuid(),project_id uuid not null references public.cloud_projects(id)on delete cascade,owner_profile_id uuid not null references public.profiles(id)on delete cascade,page_id uuid,panel_id uuid,asset_id uuid,generation_job_id uuid,panel_design_revision bigint check(panel_design_revision>0),schema_version integer not null default 1 check(schema_version=1),evaluator_id text not null,evaluator_version text not null,evaluator_kind text not null check(evaluator_kind in('rule','vision','hybrid')),data_handling text not null check(data_handling in('none','local','external')),input_snapshot jsonb not null default'{}',created_at timestamptz not null default now(),foreign key(page_id,project_id)references public.cloud_pages(id,project_id)on delete cascade);
create table if not exists public.cloud_manga_inspection_findings(id uuid primary key default gen_random_uuid(),run_id uuid not null references public.cloud_manga_inspection_runs(id)on delete cascade,project_id uuid not null references public.cloud_projects(id)on delete cascade,owner_profile_id uuid not null references public.profiles(id)on delete cascade,page_id uuid,panel_id uuid,status text not null check(status in('PASS','WARNING','FAIL','NOT_EVALUATED')),category text not null,reason text not null,region jsonb,confidence numeric(6,5)check(confidence between 0 and 1),suggestion text not null check(suggestion in('review','edit_text','regenerate_panel','inpaint','update_design','update_reference')),evidence jsonb not null default'{}',created_at timestamptz not null default now(),check((status='NOT_EVALUATED'and confidence is null)or(status<>'NOT_EVALUATED'and confidence is not null)),foreign key(page_id,project_id)references public.cloud_pages(id,project_id)on delete cascade);
create index if not exists cloud_manga_inspection_runs_project_idx on public.cloud_manga_inspection_runs(project_id,created_at desc);create index if not exists cloud_manga_inspection_findings_panel_idx on public.cloud_manga_inspection_findings(project_id,page_id,panel_id,status,created_at desc);alter table public.cloud_manga_inspection_runs enable row level security;alter table public.cloud_manga_inspection_findings enable row level security;grant select on public.cloud_manga_inspection_runs,public.cloud_manga_inspection_findings to authenticated;grant select,insert on public.cloud_manga_inspection_runs,public.cloud_manga_inspection_findings to service_role;drop policy if exists "cloud_manga_inspection_runs_owner_read" on public.cloud_manga_inspection_runs;create policy "cloud_manga_inspection_runs_owner_read" on public.cloud_manga_inspection_runs for select using(owner_profile_id=public.current_profile_id()and public.cloud_project_can_edit(project_id));drop policy if exists "cloud_manga_inspection_findings_owner_read" on public.cloud_manga_inspection_findings;create policy "cloud_manga_inspection_findings_owner_read" on public.cloud_manga_inspection_findings for select using(owner_profile_id=public.current_profile_id()and public.cloud_project_can_edit(project_id));
create or replace function public.record_cloud_manga_inspection_run(p_project_id uuid,p_page_id uuid,p_panel_id uuid,p_asset_id uuid,p_generation_job_id uuid,p_panel_design_revision bigint,p_evaluator jsonb,p_input_snapshot jsonb,p_findings jsonb)returns uuid language plpgsql security definer set search_path=public,pg_temp as $$declare v_owner uuid;v_run uuid;v_finding jsonb;begin if auth.role()<>'service_role'then raise exception'service_role_required';end if;select owner_profile_id into v_owner from public.cloud_projects where id=p_project_id;if v_owner is null or jsonb_typeof(p_findings)<>'array'then raise exception'cloud_manga_inspection_run_invalid';end if;insert into public.cloud_manga_inspection_runs(project_id,owner_profile_id,page_id,panel_id,asset_id,generation_job_id,panel_design_revision,evaluator_id,evaluator_version,evaluator_kind,data_handling,input_snapshot)values(p_project_id,v_owner,p_page_id,p_panel_id,p_asset_id,p_generation_job_id,p_panel_design_revision,p_evaluator->>'id',p_evaluator->>'version',p_evaluator->>'kind',p_evaluator->>'dataHandling',coalesce(p_input_snapshot,'{}'))returning id into v_run;for v_finding in select value from jsonb_array_elements(p_findings)loop insert into public.cloud_manga_inspection_findings(run_id,project_id,owner_profile_id,page_id,panel_id,status,category,reason,region,confidence,suggestion,evidence)values(v_run,p_project_id,v_owner,p_page_id,p_panel_id,v_finding->>'status',v_finding->>'category',v_finding->>'reason',v_finding->'region',case when v_finding->>'status'='NOT_EVALUATED'then null else(v_finding->>'confidence')::numeric end,v_finding->>'suggestion',coalesce(v_finding->'evidence','{}'));end loop;return v_run;end$$;revoke all on function public.record_cloud_manga_inspection_run(uuid,uuid,uuid,uuid,uuid,bigint,jsonb,jsonb,jsonb)from public,anon,authenticated;grant execute on function public.record_cloud_manga_inspection_run(uuid,uuid,uuid,uuid,uuid,bigint,jsonb,jsonb,jsonb)to service_role;
...
create table if not exists public.cloud_admin_generation_quality_reviews (
  ...
  reviewed_at timestamptz not null default now(),updated_at timestamptz not null default now(),
  foreign key(generation_job_id,project_id) references public.cloud_generation_jobs(id,project_id) on delete cascade
);
create index if not exists cloud_admin_generation_quality_reviews_status_idx
...
drop policy if exists "cloud_admin_generation_quality_reviews_admin_read" on public.cloud_admin_generation_quality_reviews;
create policy "cloud_admin_generation_quality_reviews_admin_read"
  on public.cloud_admin_generation_quality_reviews for select using(public.is_admin());
create or replace function public.review_cloud_admin_generation_quality(p_generation_job_id uuid,p_status text,p_note text default '')
returns uuid language plpgsql security definer set search_path=public,pg_temp as $$
declare v_actor uuid:=public.current_profile_id();v_job public.cloud_generation_jobs%rowtype;v_before_status text;v_now timestamptz:=clock_timestamp();
begin
  if v_actor is null or not exists(select 1 from public.profiles where id=v_actor and role='admin')
    or p_status not in('approved','needs_review','quality_issue') or char_length(coalesce(p_note,''))>1000
  then raise exception 'cloud_admin_generation_quality_review_invalid';end if;
  select * into v_job from public.cloud_generation_jobs where id=p_generation_job_id for update;
  if v_job.id is null or v_job.kind<>'image' or v_job.status<>'completed' or v_job.output_asset_id is null
    or not exists(select 1 from public.cloud_assets asset where asset.id=v_job.output_asset_id
      and asset.project_id=v_job.project_id and asset.source_generation_job_id=v_job.id and asset.deleted_at is null)
  then raise exception 'cloud_admin_generation_quality_review_target_invalid';end if;
  select status into v_before_status from public.cloud_admin_generation_quality_reviews where generation_job_id=v_job.id;
  insert into public.cloud_admin_generation_quality_reviews(generation_job_id,project_id,asset_id,reviewer_profile_id,status,note,reviewed_at,updated_at)
    values(v_job.id,v_job.project_id,v_job.output_asset_id,v_actor,p_status,coalesce(p_note,''),v_now,v_now)
  on conflict(generation_job_id) do update set project_id=excluded.project_id,asset_id=excluded.asset_id,
    reviewer_profile_id=excluded.reviewer_profile_id,status=excluded.status,note=excluded.note,
    reviewed_at=excluded.reviewed_at,updated_at=excluded.updated_at;
  insert into public.cloud_ai_admin_audit_logs(actor_profile_id,action,target_type,target_id,before_value,after_value)
    values(v_actor,'generation_quality_reviewed','cloud_generation_job',v_job.id::text,
...
revoke all on function public.review_cloud_admin_generation_quality(uuid,text,text) from public,anon;
grant execute on function public.review_cloud_admin_generation_quality(uuid,text,text) to authenticated,service_role;
