...
alter table public.cloud_export_jobs add constraint cloud_export_jobs_format_check check(format in('pdf','images','project_json'));
update storage.buckets set allowed_mime_types=array['application/pdf','image/png','application/zip','application/json']::text[] where id='cloud-exports';
...
begin
  if v_profile is null or p_format not in('pdf','images','project_json') or not public.cloud_project_can_edit(p_project_id) then raise exception 'cloud_export_invalid';end if;
