...
alter table public.cloud_projects
  ...
  add constraint cloud_projects_content_class_check
  ...
  add constraint cloud_projects_age_rating_check
    check(age_rating in('全年齢','12歳以上','15歳以上','18歳以上'));

alter table public.cloud_story_storyboard_projects
  ...
  where project.id=p_project_id
    and(
      ...
      or(
        ...
        and public.can_use_cloud_adult_storyboard()
      )
    )
);
$$;
...
with check(
  ...
  and(
    ...
    or(content_class='adult' and public.can_use_cloud_adult_storyboard())
  )
);
create policy "cloud_projects_update" on public.cloud_projects for update
using(public.cloud_project_can_edit(id))
with check(
...
on public.cloud_story_storyboard_projects for select using(
  owner_profile_id=public.current_profile_id()
  and(
    content_class='general'
    or(content_class='adult' and public.can_use_cloud_adult_storyboard())
...
begin
  ...
  join public.cloud_story_scenario_versions scenario
    on scenario.id=storyboard.scenario_version_id
      and scenario.content_class=storyboard.content_class
  join public.cloud_market_research_reports report
    on report.id=scenario.research_report_id
      and report.input->>'contentClass'=storyboard.content_class
  where storyboard.id=p_storyboard_version_id
    and storyboard.owner_profile_id=v_profile_id;
  if not found or v_content_class not in('general','adult') then
    raise exception 'adopted_storyboard_required';
  end if;
  if v_content_class='adult' and not public.can_use_cloud_adult_storyboard()
  then raise exception 'adult_canvas_not_allowed';end if;
  if not exists(
  ...
  if v_content_class='general' then
    ...
    insert into public.cloud_projects(
      ...
      v_project_id,v_profile_id,'cloud','adult',
      trim(v_storyboard_result->>'title'),
      '採用成人向けネームから作成した編集用Canvas下書きです。AI画像は未生成です。',
      '18歳以上','rtl',1600,2400,300
    );
    ...
    ) values(v_page_id,v_project_id,v_episode_id,1,0,1600,2400);
    insert into public.cloud_canvas_snapshots(
      ...
      jsonb_build_object(
        'schemaVersion',1,'pageId',v_page_id,'width',1600,'height',2400,
        'backgroundColor','#ffffff','panels',jsonb_build_array(),
        'panelLayers',jsonb_build_array(),'balloons',jsonb_build_array(),
    ...
    insert into public.cloud_project_versions(
      ...
      v_project_id,0,
      jsonb_build_object(
        'event','adult_storyboard_project_created',
        'episodeId',v_episode_id,'pageId',v_page_id,'contentClass','adult'
      ),v_profile_id
    );
  end if;
  ...
  insert into public.cloud_project_versions(
    ...
    v_project_id,v_revision,jsonb_build_object(
      'event','storyboard_materialized',
      'storyboardVersionId',v_storyboard_id,
      'pageCount',v_page_index,
      'contentClass',v_content_class
    ),v_profile_id
  );
  insert into public.cloud_story_storyboard_projects(
    owner_profile_id,storyboard_version_id,project_id,first_page_id,content_class
  ) values(
    v_profile_id,v_storyboard_id,v_project_id,v_first_page_id,v_content_class
  );
