...
create table public.cloud_story_proposal_runs (
  ...
  result jsonb not null check (
    ...
    and result->>'engineVersion' = 'proposal-rules-v1'
  ...
  engine_version text not null check (engine_version = 'proposal-rules-v1'),
...
create table public.cloud_story_proposal_selections (
  ...
  candidate_id text not null check (
    candidate_id in ('candidate-balanced','candidate-differentiated','candidate-focused')
  ),
    ...
    where run.id = proposal_run_id
      ...
      and exists (
        ...
        where candidate->>'id' = candidate_id
          and candidate = candidate_snapshot
      )
  )
);
