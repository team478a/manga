...
check (feature_key in ('adult_planning','adult_ai_planning','adult_scenario','adult_storyboard'));
...
begin
  ...
  if p_feature_key not in('adult_planning','adult_ai_planning','adult_scenario','adult_storyboard')
  ...
  select to_jsonb(grant_record) into v_before from public.cloud_adult_feature_grants grant_record
    where grant_record.profile_id=p_target_profile_id and grant_record.feature_key=p_feature_key;
  insert into public.cloud_adult_feature_grants(
    profile_id,feature_key,status,source,granted_by_profile_id,valid_until,admin_note
  ) values(
    p_target_profile_id,p_feature_key,p_status,p_source,p_actor_profile_id,p_valid_until,nullif(p_admin_note,'')
  ) on conflict(profile_id,feature_key) do update set
    status=excluded.status,source=excluded.source,
    granted_by_profile_id=excluded.granted_by_profile_id,
    valid_until=excluded.valid_until,admin_note=excluded.admin_note,updated_at=now();
  select to_jsonb(grant_record) into v_after from public.cloud_adult_feature_grants grant_record
    where grant_record.profile_id=p_target_profile_id and grant_record.feature_key=p_feature_key;
  v_action:=case when p_status='suspended' then 'suspend_feature'
...
select p_feature_key in ('adult_planning','adult_ai_planning','adult_scenario','adult_storyboard')
and public.can_use_cloud_adult_research()
and exists (
  select 1 from public.cloud_adult_feature_grants grant_record
...
create table public.cloud_adult_storyboard_settings(
  singleton boolean primary key default true check(singleton),
  enabled boolean not null default false,
  updated_by_profile_id uuid references public.profiles(id) on delete set null,
  updated_at timestamptz not null default now()
);
insert into public.cloud_adult_storyboard_settings(singleton,enabled) values(true,false);
...
create policy "cloud_adult_storyboard_settings_read"
on public.cloud_adult_storyboard_settings for select using(true);

create table public.cloud_adult_storyboard_consents(
  ...
  terms_version text not null check(terms_version='adult-ai-storyboard-v1'),
...
create policy "cloud_adult_storyboard_consents_owner_read"
on public.cloud_adult_storyboard_consents for select
using(profile_id=public.current_profile_id() or public.is_admin());
create policy "cloud_adult_storyboard_consents_owner_insert"
on public.cloud_adult_storyboard_consents for insert
with check(profile_id=public.current_profile_id());
create policy "cloud_adult_storyboard_consents_owner_update"
on public.cloud_adult_storyboard_consents for update
using(profile_id=public.current_profile_id())
with check(profile_id=public.current_profile_id());

create or replace function public.can_use_cloud_adult_storyboard()
returns boolean language sql stable security definer set search_path=public as $$
select public.can_use_cloud_adult_scenario()
and public.can_use_cloud_adult_feature('adult_storyboard')
and exists(select 1 from public.cloud_adult_storyboard_settings settings where settings.singleton and settings.enabled)
and exists(
  select 1 from public.cloud_adult_storyboard_consents consent
  where consent.profile_id=public.current_profile_id()
    and consent.terms_version='adult-ai-storyboard-v1'
...
create or replace function public.set_cloud_adult_storyboard_enabled(
  p_actor_profile_id uuid,p_enabled boolean
) returns void language plpgsql security definer set search_path=public as $$
begin
  if auth.role()<>'service_role' or not exists(
    select 1 from public.profiles where id=p_actor_profile_id and role='admin'
  ) then raise exception 'cloud_adult_storyboard_admin_required'; end if;
  update public.cloud_adult_storyboard_settings
...
on public.cloud_story_storyboard_versions for select using(
...
on public.cloud_story_storyboard_versions for insert with check(
  ...
  and(parent_version_id is null or exists(
    ...
    where parent.id=parent_version_id
      ...
      and parent.content_class=cloud_story_storyboard_versions.content_class
  ))
);
...
on public.cloud_story_storyboard_adoptions for select using(
  owner_profile_id=public.current_profile_id()
  and exists(
    select 1 from public.cloud_story_storyboard_versions version
    where version.id=storyboard_version_id
      and(version.content_class='general' or public.can_use_cloud_adult_storyboard())
  )
);
