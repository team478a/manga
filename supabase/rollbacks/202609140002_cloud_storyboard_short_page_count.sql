...
begin
  if exists (
    select 1
    from public.cloud_story_storyboard_versions
    where jsonb_typeof(result->'pages') = 'array'
      and jsonb_array_length(result->'pages') < 8
  ) then
    raise exception 'cloud_storyboard_short_page_count_rollback_requires_no_short_storyboards';
  end if;
end
$$;
