...
import { GenerationRouter } from "./routing/generation-router.js";
...
type AIServiceStoreMethod =
  | "addChatMessage"
  | "beginGenerationJobAttempt"
  | "bundle"
  | "changeGenerationJobPriority"
  | "confirmAdultGeneration18Plus"
  | "createApprovedExternalGenerationJob"
  | "createChatSession"
  | "createGenerationJob"
  | "createGenerationRouteDecision"
  | "deleteAsset"
  | "externalCostReservationForJob"
  | "externalCostSummary"
  | "getAdultGenerationSettings"
  | "getAdultProviderApproval"
  | "getAdultProviderPolicyState"
  | "getAdultReferenceImageAssessment"
  | "getCharacterProfile"
  | "getComfyWorkflow"
  | "getGenerationJob"
  | "getGenerationQueueSettings"
  | "getProviderSettings"
  | "getProjectGenerationPolicy"
  | "listAIModels"
  ...
  | "projectContext"
  | "projectIdForPage"
  | "readGenerationInputAsset"
  | "recordCreatedAssetsHistory"
  | "registerGeneratedAsset"
  | "releaseExternalCostReservation"
  | "releaseExternalCostReservationForJob"
  | "requeueGenerationJob"
  | "reserveExternalCost"
  | "revokeAdultGenerationConsent"
  | "saveAssetLibraryMetadata"
  | "saveGenerationQueueSettings"
  | "setAdultGenerationAdministratorEnabled"
  | "setGenerationJobStatus"
  | "settleExternalCost"
  | "settleExternalCostForJob"
  | "stopPendingAdultDezgoJobsDisallowedByPolicy"
  | "stopUnauthorizedPendingAdultGenerationJobs"
  | "updateGenerationJob";

export type AIServiceStore = Pick<MangaiDatabase, AIServiceStoreMethod>;
...
export class AIService {
  ...
  private localQueue: LocalGenerationQueue;
  private dezgoQueue: DezgoGenerationQueue;
  constructor(
    private store: AIServiceStore,
    options: {
      allowMock?: boolean;
    ...
    this.generationRouter = new GenerationRouter(this.store);
    this.localQueue = new LocalGenerationQueue({
      getSettings: () => this.store.getGenerationQueueSettings(),
      run: () => this.runNextQueuedImage(),
    });
    this.dezgoQueue = new DezgoGenerationQueue({
      getSettings: () => this.store.getGenerationQueueSettings(),
      run: () => this.runNextQueuedDezgo(),
    });
  }
  isMockEnabled() {
  ...
  private routeImageGeneration(
    ...
    const decision = this.generationRouter.decideAndRecord({
      jobId,
      projectId: input.projectId,
      prompt: input.prompt,
      draft,
  ...
  async confirmExternalSafeAsset(input: ExternalDispatchConfirmation) {
  ...
  async confirmAndEnqueueExternalSafeAsset(
    ...
    try {
      ...
      const decision = this.generationRouter.decide(draft, context);
      if (decision.blocked || decision.target !== "cloud")
        throw new Error(
          "現在のProjectポリシーではDezgo Queueへ追加できません。",
        );
      ...
      if (this.dezgoFeatures.dezgoDispatchEnabled) this.dezgoQueue.wakeSoon();
      return {
        jobId,
  ...
  resumeImageJob(jobId: string) {
    ...
    if (changed) {
      if (job.provider_id === "dezgo") this.dezgoQueue.resetAndRun();
      else this.localQueue.resetAndRun();
    }
    return changed;
  }
  ...
  resumeQueuedImages() {
    this.localQueue.resetAndRun();
    this.dezgoQueue.resetAndRun();
  }
  getQueueSettings() {
  ...
  async runNextQueuedDezgo() {
    if (!this.dezgoFeatures.dezgoDispatchEnabled || this.activeDezgoJobId)
      return null;
    this.store.stopUnauthorizedPendingAdultGenerationJobs();
    this.store.stopPendingAdultDezgoJobsDisallowedByPolicy();
    const windowDelay = this.dezgoQueue.windowDelayMs();
    if (windowDelay > 0) {
      this.dezgoQueue.schedule(windowDelay);
      return null;
    }
    const next = this.store.nextQueuedImageJob(["dezgo"]);
    if (!next) {
      const delay = this.store.nextQueuedImageDelayMs(["dezgo"]);
      if (delay !== null) this.dezgoQueue.schedule(delay);
      return null;
    }
    ...
    try {
      ...
      if (!providerResponded && disposition === "retry") {
        ...
        if (requeued) {
          this.dezgoQueue.schedule(delayMs);
  ...
  private async runNextQueuedImage() {
    this.store.stopUnauthorizedPendingAdultGenerationJobs();
    const windowDelay = this.localQueue.windowDelayMs();
    if (windowDelay > 0) {
      this.localQueue.schedule(windowDelay);
      return;
    }
    if (
    ...
    if (!next) {
      ...
      if (delay !== null) this.localQueue.schedule(delay);
      return;
    }
    this.localQueue.cancelScheduledWake();
    try {
  ...
  async generateImage(input: ImageJobRequest, existingJobId?: string) {
    ...
    const outsideQueueWindow = this.localQueue.windowDelayMs() > 0;
    if (
      this.activeLocalImageJobId ||
    ...
    try {
      ...
      if (runtime?.limits.exclusiveGpuWork && this.activeLocalTextRequests.size)
        throw new AIProviderError(
          "LOCAL_RESOURCE_BUSY",
      ...
      if (
        ...
        const delayMs = generationRetryDelayMs(
        ...
        if (
          ...
          this.localQueue.schedule(delayMs);
