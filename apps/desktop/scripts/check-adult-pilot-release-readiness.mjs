...
const paths = {
  ...
  hardware: fromEnv(
    "MANGAI_PHASE5_HARDWARE_STATUS_PATH",
...
const read = (file, label) => {
  try {
    ...
    console.error(
      `Desktop Adult pilot readiness invalid: ${label} could not be read`,
    );
    process.exit(1);
  }
};
...
const rc = read(paths.rc, "RC status"),
  bundle = read(paths.bundle, "bundle"),
  hardware = read(paths.hardware, "hardware"),
  approvals = read(paths.approvals, "approvals");
if (
  approvals.format !== "mangai.desktop-adult-pilot-release-approvals" ||
  approvals.version !== 1
) {
  console.error(
    "Desktop Adult pilot readiness invalid: approvals format is unsupported",
  );
  process.exit(1);
}
...
const hardwareReady =
  ...
  requiredOps.every((operation) =>
    profile.evidence?.some(
      (item) => item.operation === operation && item.result === "passed",
    ),
  );
const sha256Pattern = /^[0-9a-f]{64}$/;
const completion = profile?.stage0Completion;
const stage0CompletionReady =
  completion?.status === "passed" &&
  sha256Pattern.test(completion.completionSha256 ?? "") &&
  sha256Pattern.test(completion.operationPackageSha256 ?? "") &&
  sha256Pattern.test(completion.startReceiptSha256 ?? "") &&
  sha256Pattern.test(completion.hardwareEvidenceSha256 ?? "") &&
...
const approvalsReady =
  ...
  timestamp(approvals.approvedAt);
const checks = [
  [
    "signed_artifacts",
    rcStatus("windows-code-signing") === "passed" &&
      rcStatus("signed-auto-update") === "passed",
  ],
  ["fixed_bundle", bundleReady],
  ["hardware_12gb_four_modes", hardwareReady],
  ["stage0_completion_linked", stage0CompletionReady],
  [
    "consent_and_local_boundary",
    exists(
      "docs/desktop/DESKTOP_ADULT_PRIVATE_MONITOR_RELEASE_PLAN_20260831.md",
    ),
  ],
  ...
  [
    "invite_ledger",
    exists("apps/desktop/scripts/check-adult-pilot-invite-ledger.mjs") &&
      exists("docs/desktop/DESKTOP_ADULT_PILOT_INVITE_LEDGER.example.json"),
  ],
...
console.log(
  `Desktop Adult pilot release readiness: ready=${checks.length - blocked.length}, blocked=${blocked.length}`,
);
