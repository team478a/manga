...
test("追加migrationは旧versionを停止して4用途を最大4MPの$0.180で予約しrollbackで復元する", async () => {
  ...
  assert.match(migration, /pricing_version <> 'bfl-flux2-pro-2026-08'/);
  ...
  assert.match(rollback, /set active = true/);
  assert.match(rollback, /pricing_version = 'bfl-flux2-2026-03'/);
});
