...
import { checkCloudRelease5Environment } from "../scripts/check-cloud-release5-preflight.mjs";
...
test("Release 5 preflightはCanvas flag未設定時fail closedする", () => {
  assert.equal(
    checkCloudRelease5Environment({
      ...valid,
      CLOUD_STORYBOARD_CANVAS_ENABLED: undefined,
    }).passed,
    false,
  );
  assert.equal(checkCloudRelease5Environment(valid).passed, true);
});
